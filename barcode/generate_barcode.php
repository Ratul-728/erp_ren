...
?>
                     <div class="parent allInOnePage">
					<div class="printit oneInOnePage">
                     <?php
...
<?php 
                                ...
                                $itemTitle = substr($prd, 0, 45);
                                $partsbc=$bc."-".$partsno;
							    echo '<div class="printit oneInOnePage"><div class="itemname">'.$prdnm.'</div>';	
							    echo '<div class="almas">'.$brand.'</div>';
							    echo '<div class="almas">'.$itmcat.'</div>';
...
<br>    
<input class="btn btn-lg btn-primary" type="button" name="print_allInOnePage" value="Print All In One Page">
<input class="btn btn-lg btn-primary" type="button" name="print_oneInOnePage" value="Print One In One Page">
</div>
...
$(document).ready(function () {
    // Print All In One Page - all .printit sections as one image
    $('input[name="print_allInOnePage"]').on('click', function () {
        html2canvas($('.allInOnePage')[0], {
            scale: 2 // Higher scale for better resolution
        }).then(function (canvas) {
            let imageData = canvas.toDataURL('image/png', 1.0);
            let img = new Image();
            img.src = imageData;
            img.style.width = "400px"; // Set the image to 400px width
    ...
    // Print Each One In One Page - each .oneInOnePage as separate image
    $('input[name="print_oneInOnePage"]').on('click', function () {
        $('.oneInOnePage').each(function (index) {
            html2canvas(this, {
                scale: 2
            }).then(function (canvas) {
                let imageData = canvas.toDataURL('image/png', 1.0);
...
$(document).ready(function () {
    ...
    // Print all .printit sections as one page in a PDF
    $('input[name="print_allInOnePage"]').on('click', function () {
        let pdf = new jsPDF();
        ...
        html2canvas($('.allInOnePage')[0], {
            ...
            pdf.save('allInOnePage.pdf');
        });
    });

    // Print each .oneInOnePage section as separate pages in one PDF
    $('input[name="print_oneInOnePage"]').on('click', function () {
        let pdf = new jsPDF();
        let promises = [];
        ...
        $('.oneInOnePage').each(function (index) {
            let element = this;
            let promise = html2canvas(element, {
                scale: 2 // Scale for better quality
            }).then(function (canvas) {
                let imgData = canvas.toDataURL('image/png');
                let imgWidth = 210; // A4 width in mm
                ...
                // Add the image to a new page in the PDF
                if (index > 0) {
                    pdf.addPage(); // Add a new page for each .oneInOnePage after the first
                }
                pdf.addImage(imgData, 'PNG', 0, 0, imgWidth, imgHeight);
            });
        ...
        // After all pages are added, save the PDF
        Promise.all(promises).then(function () {
            pdf.save('oneInOnePage.pdf');
        });
    });
});
