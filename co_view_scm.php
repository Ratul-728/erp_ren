...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1>CO TRANSFER STOCK</h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
<hr>
			<?php
			$qry = "SELECT org.name orgnm, date_format(q.orderdate,'%d/%b/%Y') orderdate, c.order_id,c.co_id, d.do_id
                                FROM delivery_order d LEFT JOIN `co` c ON c.co_id=d.order_id LEFT JOIN quotation q ON c.order_id=q.socode LEFT JOIN organization org ON org.id=q.organization
                                WHERE d.do_id = '$doid'";
