...
if ($usr == '') {
	...
	$qry = "SELECT m.`do_number`,m.date, mt.name,m.fee,m.tds,m.vds,m.total, o.orgcode 
            FROM `maintenance` m LEFT JOIN maintenance_type mt ON mt.id=m.reason LEFT JOIN delivery_order d ON m.do_number=d.do_id LEFT JOIN quotation q ON q.socode=d.order_id 
...
$quotDetailQry ="SELECT i.name item, i.code,sod.qty,i.image
                 FROM maintenance_details sod LEFT JOIN item i ON i.id=sod.product WHERE sod.code = '" . $code. "' ORDER BY sod.id ASC";	
