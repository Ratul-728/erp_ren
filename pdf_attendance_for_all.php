...
$attendance_from = $_GET["dt_f"];
$attendance_to = $_GET["dt_t"];

if($attendance_from == '') $attendance_from = '2024-01-01';
if($attendance_to == '') $attendance_to =  date('Y-m-d');
    ...
    <div id="wrapper"> 
      ...
      <div id="page-content-wrapper">
        ...
        <span class="alertmsg">
			...
			<?php 
			    $qry1="SELECT concat(emp.firstname, ' ', emp.lastname, '-(', employeecode, ')') empnm, h.attendance_id, emp.id 
			            FROM `employee` emp LEFT JOIN hr h ON h.emp_id=emp.employeecode 
			            WHERE h.attendance_id is NOT null AND h.attendance_id > 0 AND h.active_st = 1
			            ORDER by empnm";
			         //    and  h.attendance_id != 84 and  h.attendance_id != 86 and  h.attendance_id != 82 and  h.attendance_id != 97 and  h.attendance_id != 89
			         //   and  h.attendance_id != 95 and  h.attendance_id != 74 and  h.attendance_id != 70 and  h.attendance_id != 80
            	$result1 = $conn->query($qry1); 
            	while($row1 = $result1->fetch_assoc()) 
                {    $emp_att_id= $row1["attendance_id"];     
			...
			<?php $qryInfo = "SELECT des.name des, dep.name dep, ja.Title jobarea, emp.employeecode, concat(emp.firstname, ' ', emp.lastname) empnm, emp.tin, emp.nid,
			                  DATE_FORMAT(emp.dob, '%d/%b/%Y') dob, emp.photo, h.id hrid
			...
			?>
				...
				?>
				        <tr>
				            ...
				            <td class="status"><?= $rowAtt["stats"] ?></td>
