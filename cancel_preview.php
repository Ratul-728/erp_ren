...
from cancel_order ro LEFT JOIN invoice inv ON inv.soid=ro.order_id
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1><?= strtoupper('Cancel Order') ?></h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
-->
		...
		<td valign="top">
			<h5>Cancel Order Informaiton<br></h5>
			
			<p>
			    <h5>Cancel ID:  <?=$roid?></h5>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-items">
  <tbody>
    <tr class="inv-header">
      ...
      <th scope="col">Cancelled Qty</th>
    </tr>
	  
	  
<?php
$dqry   = "SELECT ro.id,ro.co_id, ro.order_id,  DATE_FORMAT(q.orderdate, '%d/%b/%Y') orderdate, org.name customer,i.name product,qd.qty orderqty,ro.qty_canceled 
,( case when ro.st=1 then 'Pending' when ro.st=2 then 'Approved' else 'Decliend' end) apprvst 
,ro.st st,i.image,i.description
...
if ($resultd->num_rows > 0) {
	...
	//	$dimension                       = $rowsd["dimension"];
        ...
        $pnm                             = $rowsd["product"];
        $orderqty                        = $rowsd["orderqty"];
        $returnqty                       = $rowsd["qty_canceled"];
