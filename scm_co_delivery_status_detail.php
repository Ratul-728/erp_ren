...
else
{
    ...
    $currSection = 'deliverycostatusList';
    ...
    $qry = "SELECT org.name orgnm, date_format(q.orderdate,'%d/%b/%Y') orderdate, c.order_id,c.co_id, d.do_id, rp.delivery_start,d.id
                                FROM delivery_order d LEFT JOIN `co` c ON c.co_id=d.order_id LEFT JOIN quotation q ON c.order_id=q.socode 
                                LEFT JOIN organization org ON org.id=q.organization LEFT JOIN `resourceplan` rp ON d.do_id = rp.doid
                                WHERE d.do_id = '$deliveryId'";
                  ...
                  <div class="well table-header">
                       ...
                       <div class="row">
                           <div class="col-sm-6">
                                <div class="row">
                                   <div class="col-md-6">
                                           <b>Delivery Number:  </b>  <?= $deliveryId ?> <br>
                                           <b>CO Number: <?= $co_id ?></b> <br>
                                           <b>Order Number: <?= $socode ?></b> <br>
                                    </div>
                           ...
                           <div class="col-sm-6">
                                ...
                                <div class="row">
                                   <div class="col-md-6">
                                           <b>Order Date: <?= $orderdt ?></b> <br>
                                            <b>DELIVERY DATE:  <?= $deli_start ?>  </b>  <br>
                                    </div>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td><!-- QA Status by Warehouse -->
                                        <table width="100%" class="qsw-tbl"  cellpadding="6" cellspacing="6">
                                            <tr>
                                                <td class="qsw-1 qsw-head">From Warehouse</td>
                                                <td class="qsw-6 qsw-head">To Warehouse</td>
                                    ...
                                    <?php 
                                        $qryQaw = "SELECT  dod.do_qty,dod.id, dod.st, dod.pending_qty, dod.intransit_qty, dod.delivered_qty, dod.due_return_qty,
                                                    dod.returned_qty, frombr.name fromstore, tobr.name tostore 
                                                    FROM `delivery_order_detail` dod LEFT JOIN co_details cd ON cd.id=dod.qa_id LEFT JOIN branch frombr ON frombr.id=cd.before_warehouse 
                                                    LEFT JOIN branch tobr ON tobr.id=cd.warehouse_id WHERE dod.do_id =".$doId." AND dod.item = ".$itemId; //echo $qryQaw;die;
                                        ...
                                        while ($rowQaw = $resultQaw->fetch_assoc()) {
                                            ...
                                            $fromstore = $rowQaw["fromstore"];
                                            $tostore = $rowQaw["tostore"];

                                            

                                    ?>
                                            <tr class="ds-row-wrapper" id="row_<?=$dodId?>">
                                                <th class="text-right"><?= $fromstore ?> (<?= $doQty ?>)
                                                <th class="text-right"><?= $tostore ?> (<?= $doQty ?>)
...
$(".plus_intransit, .plus_delivery").on("click",function(){
  ...
  if(isChanged){
  $.ajax({
        url:"phpajax/updateCODeliverySt.php",
        method:"POST",
