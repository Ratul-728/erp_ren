...
if($_REQUEST['action']=='restore'){	
    ...
    if ($res == 4) { //update mode
        ...
        $qry = "SELECT s.`id`, s.orderstatus, s.`socode`,s.`organization`, s.`customer`,DATE_FORMAT(s.`orderdate`,'%e/%c/%Y') `orderdate`,s.`deliveryamt`,s.`accmanager`, s.`vat`, s.`tax`, s.`invoiceamount`, s.`makeby`, s.`makedt`,s.`status`,s.`remarks`,s.`poc`,s.`oldsocode`,DATE_FORMAT(s.mrcdt,'%e/%c/%Y') mrcdt, o.name orgname,adjustment
            FROM `quotation` s 
        ...
        <div class="container-fluid pagetop">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <p>
                       <form method="post" action="common/quotation_post.php"  id="Quotationform"  enctype="multipart/form-data">
...
<?php if($mode == 1) { //insert ?>
    ...
    <?php }} ?>
												...
												<div class="col-lg-1 col-md-1 col-sm-1 col-xs-4">
													...
													<div class="form-group">
														<input  type="text"  required class="numonly calc c-qty form-control quantity_otc_ qty-chkstk" id="quantity_otc_" placeholder="Qty" name="quantity_otc[]">
													</div>
												</div>
                                                ...
                                                <!-- this block is for discount-->
                                                 <div class="col-lg-1 col-md-1 col-sm-1  col-xs-2">
													...
													<div class="form-group">
														<input type="number" min="0" max="100"  class="numonly calc c-discount form-control discnt_" id="discnt_"   placeholder="Discount %" name="discnt[]" >
													</div>
                                                </div>												
    ...
    <?php 
			...
			$resultitm = $conn->query($qryitm); if ($resultitm->num_rows > 0) {while($rowitm = $resultitm->fetch_assoc()) 
              { 
				...
				$stock=$rowitm["freeqty"];
    ?>
                                                                <option  class="option-<?=$tid?>" data-value="<?php echo $tid; ?>" data-stock="<?=$stock?>" data-prdcost="<?php echo $prdcost; ?>" data-up="<?php echo $up; ?>" data-cost="<?php echo $cost; ?>" data-vat="<?php echo $vat; ?>" data-ait="<?php echo $ait; ?>" value="<?=$nm?>-[Cd: <?=$code; ?> | St: <?=$stock?>]"><?=$nm?>-[Cd: <?=$code; ?> | St: <?=$stock?>]</option>
    <?php  }}?>                    
                                                            </datalist> 
                                                        </div>
												...
												<div class="col-lg-1 col-md-1 col-sm-5 col-xs-4">
													<label class="hidden-lg">Qty</label>
													<div class="form-group">
														<input type="text"  required class="calc c-qty form-control quantity_otc_"  id_="quantity_otc" value="<?php echo $itdqu; ?>" name="quantity_otc[]">
													</div>
												</div>	
...
?>									
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <label for="details">Revision Notes </label>

                                            <textarea class="form-control" id="details" name="details" rows="2" ><?php echo $details; ?></textarea>
                                    ...
                                    <div class="col-sm-12"> 
                                            ...
                                            //for update ?>
													...
													<input class="btn btn-lg btn-default" <?=($orderstatus == 2 or $amount==0 )?"disabled":""?> type="submit" name="postaction" value="Create Order"  id="confirm" > 
                                          ...
                                          <?php } else { // new insert ?>
											  ...
											  <!--input  dat a-to="pagetop" class="btn btn-lg btn-default" type="submit" name="addprint" value="+Add and Print Order" id="add"-->
													...
													<input class="btn btn-lg btn-default" disabled type="submit" name="postaction" value="Create Order"  id="confirm" disable > 										
										
                                          <?php } ?>

												<input  class="btn btn-lg btn-warning top" type="button" name="postaction" value="Cancel" id="cancel"  onClick="location.href = 'quotationList.php?pg=1&mod=2'" >
...
<br>
							...
							?>
												...
												<td nowrap style="white-space:nowrap">
													...
													<a class="btn btn-info btn-xs" href="quotationEntry.php?res=4&action=restore&socode=<?=$soid?>&id=<?=$_REQUEST['id']?>&rid=<?=$row['qrid']?>&mod=2&changedid=<?=$row['qrid']?>"   title="Restore"><i class="fa fa-repeat"></i></a>
												</td>
...
$(document).on("input", ".dl-itemName", function() {
  ...
  var stock =  $('#itemName option[value="' + val +'"]').attr('data-stock');
  var pid =  $('#itemName option[value="' + val +'"]').attr('data-value');
	...
	//alert(pid);
			...
			//show warehouse wise quantity;
			//if(stock>0){
				
                //alert(root.find('.qtycounter').length);
