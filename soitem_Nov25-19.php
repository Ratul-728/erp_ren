...
else
{
                ...
                if ($result->num_rows > 0)
                {
                    while($row = $result->fetch_assoc()) 
                        { 
                            ...
                            $invoice_amount=$row["invoiceamount"];$vat=$row["vat"]; $tax=$row["tax"]; $delivery_dt=$row["deliverydt"]; $term_dt=$row["terminationDate"]; $effectivedate=$row["effectivedate"];
                           $hrid='1'; $st=$row["status"]; $details=$row["remarks"];
                        }
                }
            }
    $mode=2;//update mode
   // echo "<script type='text/javascript'>alert('".$orderdt."')</script>"; 
    ...
    {
                            $uid='';$soid='';$srctype=''; $cusid=''; $orderdt='';  $currency='';$deliveryby='';$accmgr='';
                            $invoice_amount='0'; $vat='0';$tax='0'; $delivery_dt='';$hrid='';$term_dt='';$st='';$effect_dt='';$details='';//$term_dt=date("Y-m-d")
                                    ...
                                    <div class="col-lg-3 col-md-6 col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <select name="cmbcontype" id="cmbcontype" class="cmb -parent form-control" required>
													...
													<?php $qrycntp="SELECT `id`, `name` FROM `customertype`"; $resultcntp = $conn->query($qrycntp); if ($resultcntp->num_rows > 0) {while($rowcntp = $resultcntp->fetch_assoc()){
                                    ...
                                    <div class="col-lg-3 col-md-6 col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <select name="cmborg" id="cmborg" class="cmb-parent form-control">
                                                	<option value="">Select Type</option>
													<?php $qryorg="SELECT distinct o.`id`,o.`name` FROM `contact` c,`organization` o where c.`organization`=o.`id`  and c.`contacttype`=1"; $resultorg = $conn->query($qryorg); if ($resultorg->num_rows > 0) {while($roworg = $resultorg->fetch_assoc()){
      	                            ...
      	                            <div class="col-lg-3 col-md-6 col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <option value="">Select Name</option>
                                                    <?php $qrycont="SELECT `id`, `name`  FROM `contact`  WHERE `contacttype`=1"; $resultcont = $conn->query($qrycont); if ($resultcont->num_rows > 0) {while($rowcont = $resultcont->fetch_assoc()){
...
<?php $qryhrm="SELECT h.`id`,concat(e.`firstname`,' ',e.`lastname`) `emp_id` FROM `hr` h,`employee` e where h.`emp_id`=e.`employeecode`"; $resulthrm = $conn->query($qryhrm); if ($resulthrm->num_rows > 0) {while($rowhrm = $resulthrm->fetch_assoc()) 
      { 
...
<?php $qryst="SELECT `id`, `name` FROM `sostatus`"; $resultst = $conn->query($qryst); if ($resultst->num_rows > 0) {while($rowst = $resultst->fetch_assoc()) 
      { 
...
<?php if($mode==1){?> 	                                        
    ...
    <?php $qryitm="SELECT `id`, `name`  FROM `item`"; $resultitm = $conn->query($qryitm); if ($resultitm->num_rows > 0) {while($rowitm = $resultitm->fetch_assoc()) 
              { 
 ...
 <?php $qryunit="SELECT `id`, `name`, `description`, `st` FROM `mu` WHERE st=1"; $resultunit = $conn->query($qryunit); if ($resultunit->num_rows > 0) {while($rowunit = $resultunit->fetch_assoc()) 
              { 
...
$itmdtqry="SELECT `id`, `socode`, `sosl`, `productid`, `mu`, round(`qty`,0) qty,round(`qtymrc`,0)qtymrc, round(`otc`,2) otc, round(`mrc`,2)mrc, `remarks`, `makeby`, `makedt` FROM `soitemdetails` WHERE `socode`='".$soid."'";
$resultitmdt = $conn->query($itmdtqry); if ($resultitmdt->num_rows > 0) {while($rowitmdt = $resultitmdt->fetch_assoc()) 
              { 
                  $itmdtid= $rowitmdt["productid"];  $itdmu=$rowitmdt["mu"]; $itdqu=$rowitmdt["qty"];$itdqumrc=$rowitmdt["qtymrc"]; $itdotc=$rowitmdt["otc"]; $itdmrc=$rowitmdt["mrc"]; $itdrem=$rowitmdt["remarks"];
...
?>                                            
                                            ...
                                            <div class="toClone">
                                                <div class="col-lg-3 col-md-6 col-sm-6">
                                                    <div class="form-group">
                                                        <div class="form-group styled-select">
                                                            <!--<input list="itemName"  autocomplete="Search From list"  class="dl-itemName datalist">-->
 ...
 <?php //and `id`=".$itdmu."
 $qrymu="SELECT `id`, `name`, `description`, `st` FROM `mu` WHERE st=1 "; $resultmu = $conn->query($qrymu); if ($resultmu->num_rows > 0) {while($rowmu = $resultmu->fetch_assoc()) 
              { 
                  $mid= $rowmu["id"];  $mnm=$rowmu["name"];
    ?>                                                          
                                                        <option value="<?php echo $mid; ?>" <?php if ($itdmu == $mid) { echo "selected"; } ?>><?php echo $mnm; ?></option>
     <?php  }}?>                                          <!-- <option data-value="<?php echo $mid; ?>" value="<?php echo $mid; ?>" <?php if ($itdmu == $mid) { echo "selected"; } ?>><?php echo $mnm; ?></option>  -->            
                                                      </select>
                                                      </div>
                                                  </div>        
                                                </div>
                                              ...
                                              <div class="row qtnrows">
                                                ...
                                                <!--div class="col-lg-3 col-md-6 col-sm-6">
                                                    <div class="form-group">
                                                        <input type="text" class="form-control" id="remarks" value="<?php echo $itdrem;?>" name="remarks[]">
                                                    </div>        
                                                </div-->                                                
