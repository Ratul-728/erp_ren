...
else
{
   ...
   if ( isset( $_POST['export'] ) ) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A1', 'SL.')
                ->setCellValue('B1', 'PACKAGE')
                ->setCellValue('C1', 'BENIFIT')
                ->setCellValue('D1', 'GRADE');
                  
    			
        $firststyle='A2';
        $qry="SELECT a.`id`, a.`Title` mtitle, p.`title` ptitle, c.`title` compansation from pakageSetup a

        left join pakage p on p.id=a.pakage LEFT JOIN compansationSetup c ON a.`scale`= c.id  "; 
       // echo  $qry;die;
        ...
        { $i=0;
            ...
            { 
                ...
                $objPHPExcel->setActiveSheetIndex(0)
    			            ->setCellValue($col1, $i)
    			            ->setCellValue($col2, $row['mtitle'])
    						->setCellValue($col3, $row['ptitle'])
    						->setCellValue($col3, $row['compansation']);
    					    	/* */
    			//$laststyle=$title;	
            }
        }
        $objPHPExcel->getActiveSheet()->setTitle('PACKAGE SETUP');
        $objPHPExcel->setActiveSheetIndex(0);
        $today=date("YmdHis");
        $fileNm="data/".'package_setup'.$today.'.xls'; 
                     ...
                     <div class="well list-top-controls"> 
                      <div class="row border">
                         ...
                         <div class="col-sm-3 text-nowrap">
                            <h6>HRM <i class="fa fa-angle-right"></i>Pakage Setup</h6>
                       </div>
