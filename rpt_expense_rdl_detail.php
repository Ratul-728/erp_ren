...
order by c.`glno`,m.yr,m.mn;
    */
    ...
    $currSection = 'rpt_expense_rdl_all';
   ...
   // $fdt = $_POST['fdt'];if ($fdt == '') {$fdt = date('d/m/Y', strtotime('-1 month'));}
    ...
    //echo $vlvl;die;
    //}
    $ajxurl = "phpajax/datagrid_list_all.php?action=rpt_expense_rdl_all&gllvl='.$vlvl.'&ctrgl=".$vctgl;
    //echo $ajxurl;
...
,nvl((select m.closingbal from `coa_mon` m where c.glno=m.glno and  m.yr='$sessyrt' and  m.mn='5'),0)may
,nvl((select m.closingbal from `coa_mon` m where c.glno=m.glno and  m.yr='$sessyrt' and  m.mn='6'),0)jun
FROM `coa` c where   substr(c.`glno`,1,1)=4 and c.lvl=3 and c.ctlgl=$pctr2  and c.oflag='N' 
order by c.`glno`";//order by '".$columnName."' '".$columnSortOrder."' limit ".$fixrow." , ".$rowperpage;
              // echo $qry1;die;
...
,nvl((select m.closingbal from `coa_mon` m where c.glno=m.glno and  m.yr='$sessyrt' and  m.mn='5'),0)may
,nvl((select m.closingbal from `coa_mon` m where c.glno=m.glno and  m.yr='$sessyrt' and  m.mn='6'),0)jun
FROM `coa` c where   substr(c.`glno`,1,1)=4 and c.lvl=4 and c.ctlgl=$pctr3  and c.oflag='N' 
order by c.`glno`";//order by '".$columnName."' '".$columnSortOrder."' limit ".$fixrow." , ".$rowperpage;
              // echo $qry1;die;
...
,nvl((select m.closingbal from `coa_mon` m where c.glno=m.glno and  m.yr='$sessyrt' and  m.mn='5'),0)may
,nvl((select m.closingbal from `coa_mon` m where c.glno=m.glno and  m.yr='$sessyrt' and  m.mn='6'),0)jun
FROM `coa` c where   substr(c.`glno`,1,1)=4 and c.lvl=5 and c.ctlgl=$pctr4  and c.oflag='N' 
order by c.`glno`";//order by '".$columnName."' '".$columnSortOrder."' limit ".$fixrow." , ".$rowperpage;
              // echo $qry1;die;
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="rpt_expense_rdl_detail.php?mod=7" id="form1">
...
function table_with_filter(url){
	...
	var yr=$("#cmbyr").val();
//	alert(lvl);
	url = 'phpajax/datagrid_list_all.php?action=rpt_expense_rdl_all&gllvl='+plvl+'&ctrgl='+pctrl+'&pyr='+yr;
	//	alert(url);
	table_with_filter(url);	
        ...
        $(".cancelBtn").click(function(){
        	...
        	url = 'phpajax/datagrid_list_all.php?action=rpt_expense_rdl_all';
        	table_with_filter(url);
        });
