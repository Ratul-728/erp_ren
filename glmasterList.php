...
else
{
    ...
    */
    ...
    if ( isset( $_POST['add'] ) ) {
           header("Location: ".$hostpath."/glmaster.php?res=0&msg='Insert Data'&mod=7");
    }
   if ( isset( $_POST['export'] ) ) {
        ...
        $firststyle='A2';
        $qry="SELECT `id`, `vouchno`, DATE_FORMAT(`transdt`,'%d/%m/%Y') `transdt`, `refno`,substring(`remarks`,1,50) remarks FROM `glmst` WHERE `status` in('1','A') "; 
       // echo  $qry;die;
        ...
        { $i=0;
            ...
            { 
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue($col3, $row["transdt"]) // Store the pure date value (without time)
                    ->setCellValue($col4, $row['refno'])
                    ->setCellValue($col5, $row['remarks']);
                
                    // Apply the date format to ensure it's shown without time
                    $objPHPExcel->getActiveSheet()
                        ->getStyle($col3)
                        ->getNumberFormat()
                        ->setFormatCode('DD/MM/YYYY'); // Ensure it shows just the date
    					     	/* */
    			$laststyle=$title;	
            }
        }
                	...
                	<form method="post" action="glmasterList.php" id="form1">
                     ...
                     <!-- <div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Accounting <i class="fa fa-angle-right"></i>All GL Master</h6>
                       </div>
                        ...
                        <div class="pull-right grid-panel form-inline">
                          <div class="form-group">
                            ...
                            <div class="form-group">
                            <input type="hidden" id="pdfsource" url="pdf_glmaster.php">
     ...
     <!-- Datatable JS -->       
        ...
        $(document).ready(function(){
           var table1 = $('#listTable').DataTable({
                ...
                "dom": "rtiplf",
                'ajax': {
                    'url':'phpajax/datagrid_list_all.php?action=glmaster'
                },
                'columns': [
                    { data: 'id' },
                    { data: 'vouchno' },
