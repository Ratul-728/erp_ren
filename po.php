...
else
{
    ...
    $qry="SELECT `id`, `poid`, `supid`, `orderdt`,`shipdt`, `currency`, `tot_amount`, `invoice_amount`, `vat`, `tax`, `delivery_dt`, `hrid` FROM `po`  where  id= ".$id; 
    //echo $qry; die;
        ...
        else
            {
                ...
                if ($result->num_rows > 0)
                {
                    while($row = $result->fetch_assoc()) 
                        { 
                            $uid=$row["id"];$poid=$row["poid"]; $supid=$row["supid"]; 
                            $orderdt=$row["orderdt"]; $date = DateTime::createFromFormat('Y-m-d', $orderdt); $orderdt = $date->format('d/m/Y');
                            
                            $currency=$row["currency"];
                            $tot_amount=$row["tot_amount"]; $invoice_amount=$row["invoice_amount"];$vat=$row["vat"]; $tax=$row["tax"]; 
                            $delivery_dt=$row["delivery_dt"]; $date = DateTime::createFromFormat('Y-m-d', $delivery_dt); $delivery_dt = $date->format('d/m/Y');
                           $hrid='1';
                           
                           $ship_dt = $row["shipdt"]; $date = DateTime::createFromFormat('Y-m-d', $ship_dt); $ship_dt = $date->format('d/m/Y');
                        }
                }
            }
    $mode=2;//update mode
...
<div id="wrapper"> 
                            ...
                            <span class="alertmsg"></span>
                                   ...
                                   <div class="row form-header"> 
	                                    ...
	                                    <div class="col-lg-6 col-md-6 col-sm-6">
      		                                <h6>Products <i class="fa fa-angle-right"></i> Add New PO</h6>
      		                            </div>
    ...
    <?php 
    $qryorg="SELECT distinct o.`id`,o.`name` FROM `contact` c,`organization` o where c.`organization`=o.`orgcode`  and c.`contacttype` in (1,3)  order by o.name" ; $resultorg = $conn->query($qryorg); if ($resultorg->num_rows > 0) {while($roworg = $resultorg->fetch_assoc()){
    ...
    <?php }}?>                    
                                    ...
                                    <div class="col-lg-3 col-md-6 col-sm-6">
	                                    <label for="po_dt">Shipping Date</label>
                                        <div class="input-group">
                                            <input type="text" class="form-control datepicker" name="ship_dt" id="ship_dt" value="<?php echo $shipdt;?>">
                                            <div class="input-group-addon">
                                                <span class="glyphicon glyphicon-th"></span>
                                            </div>
                                        </div>     
...
<?php if($mode==1){?> 	                                        
    ...
    <?php $qryitm="SELECT `id`, `name`, `description`, `st` FROM `item`  order by name"; $resultitm = $conn->query($qryitm); if ($resultitm->num_rows > 0) {while($rowitm = $resultitm->fetch_assoc()) 
              { 
 ...
 <?php $qrymu="SELECT `id`, `name`, `description`, `st` FROM `mu`  order by name"; $resultmu = $conn->query($qrymu); if ($resultmu->num_rows > 0) {while($rowmu = $resultmu->fetch_assoc()) 
              { 
...
<?php } else {?>                                            
    ...
    <?php $qry1="SELECT `id`, `name`, `description`, `st` FROM `item`  order by name"; $result1 = $conn->query($qry1); if ($result1->num_rows > 0) {while($row1 = $result1->fetch_assoc()) 
              { 
        	...
        	<div class="form-group grandTotalWrapper">
                ...
                <input type="text" class="form-control" id="grandTotal" disabled>
	                                ...
	                                <div class="col-lg-3 col-md-6 col-sm-6">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control datepicker" id="delivery_dt" name="delivery_dt" value="<?php echo $delivery_dt;?>" >
                                            <div class="input-group-addon">
                                                <span class="glyphicon glyphicon-th"></span>
                                            </div>
                                        </div>     
...
<?php $qryhr="SELECT `id`, `emp_id`, hrName FROM `hr`  order by emp_id"; $resulthr = $conn->query($qryhr); if ($resulthr->num_rows > 0) {while($rowhr = $resulthr->fetch_assoc()) 
      { 
          $hrid= $rowhr["id"];  $hrnm=$rowhr["hrName"];
?>                                                          
...
<?php  }}?>                                                       
                          ...
                          <?php } ?>
                        <a href = "http://bithut.biz/BitFlow/poList.php?pg=1&mod=1">
