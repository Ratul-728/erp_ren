...
else
{
    ...
    */
    ...
    $qry="SELECT p.`modelCode`,p.productName, p.`prodPhoto`,sum(s.`qty`) qty, avg(s.`cost`) cst, avg(p.`rate`) rate FROM `productstock` s,`product` p  WHERE s.productid=p.id group by p.`modelCode`,p.productName order by p.`modelCode` asc LIMIT ".$limitst. ",".$limitnd;
    $sl=0;
    //echo $qry; die;
