...
if ((($paymentSt == 4 || $paymentSt == 3) && $st > 1) || $approval == 1) {
//print_r($qaIds);die;

$formString = '
<div class="row">
    <form method="post" action="common/adddeliveryQA.php?dotype=sales" id="form1" enctype="multipart/form-data">
...
for ($i = 0; $i < count($qaIds); $i++) {
    $qaId = $qaIds[$i];
    $itmdtqry = "SELECT concat(i.name,' (', i.barcode, ')') productnm,i.id productid, qaw.ordered_qty, b.name warehouse, qaw.`pass_qty`, qaw.id, b.id warehouseId
    ...
    if ($resultitmdt->num_rows > 0) {
        while ($rowitmdt = $resultitmdt->fetch_assoc()) {
            ...
            //CO qty
            $coQty = 0;
            $qryCo = "SELECT `co_qty` FROM `co_details`
                     WHERE order_id = '$orderID' AND product_id = '$productId' AND before_warehouse = '$warehouseId'";
            // echo $qryCo;die;
