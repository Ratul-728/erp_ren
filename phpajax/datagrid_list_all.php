...
//.$searchQuery
...

    {
      ...
      $tdt= $_GET['dt_t'];
      if($fdt != ""){
          $date_qry = "AND m.transdt BETWEEN STR_TO_DATE('$fdt','%d/%m/%Y') AND STR_TO_DATE('$tdt','%d/%m/%Y')";
      }else{
          $date_qry = "";
      }
...

        {
        	...
        	$searchQuery = " and (c.glnm like '%".$searchValue."%' or m.VoucherTp like '%".$searchValue."%'  ) ";

        }

        ## Total number of records without filtering   #c.`id`,	and a.status='A'

        $strwithoutsearchquery="select DATE_FORMAT( m.transdt,'%d/%b/%Y') expenseDate,c.glnm expnsNature,d.dr_cr ,d.amount amount,m.VoucherTp paymode
...
WHERE q.orderstatus in( '7','8') $dateqry
    ...
    else if($action=="rpt_accounts_receivale")
    {
        ...
        if($fdt == ''){
            ...
            $dateqry = " and w.transdt <='$fdt'";
        }
        
        $searchQuery = " ";
        if($searchValue != ''){
        	$searchQuery = " and ( customercode like  '%".$searchValue."%' or ocustomernm  like '%".$searchValue."%') ";
        }
    ...
    else if($action=="rpt_accounts_payable")
    {
