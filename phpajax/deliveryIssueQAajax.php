...
//Get Info
$qaIds = [];
$qryUpperInfo = "SELECT  iw.name, ioo.iodt orderdate, qa.id, qa.status st, qa.approval, qa.type    
                FROM `qa` qa LEFT JOIN issue_order ioo ON qa.order_id = ioo.ioid LEFT JOIN issue_warehouse iw ON iw.id = ioo.issue_warehouse 
                WHERE qa.order_id = '".$orderID."'";
...
for ($i = 0; $i < count($qaIds); $i++) {
    $qaId = $qaIds[$i];
    $itmdtqry = "SELECT i.name productnm,i.id productid, qaw.ordered_qty, b.name warehouse, qaw.`pass_qty`, qaw.id 
...
echo $formString;
    ...
    echo "<script> alert('Please enter issue id') </script>";die;
}
