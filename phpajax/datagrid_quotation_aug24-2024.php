...
if($action=="quotation")
    ...
    {   
        ...
        $basequery="SELECT s.`id`, s.makedt makedt, s.`socode`, o.`name` customer, o.orgcode, s.`orderdate`, date_format(s.`orderdate`,'%d/%b/%Y') `orderdate_formated`, s.orderstatus, orst.name `quotationstatusname`
...
FROM `quotation` s left join `quotation_detail` sd on sd.socode=s.socode left join `organization` o on o.`id`=s.organization left join `invoice` inv on inv.`soid`=s.socode left join `quotation_status` orst on s.`orderstatus`=orst.`id`  WHERE 1=1 and (s.organization = $orgid or $orgid = 0) $cmbstatus_str $gift";
        /*
        $basequery=
        "SELECT 
        s.`id`,orst.sl, 
        ...
        WHERE  1=1 and (s.organization = $orgid or $orgid = 0) $cmbstatus_str $gift";*/
  ...
  // s.`status`<>6 
       ...
       /* else if ($columnName == "makedt")
        ...
        else
        {
         ...
         $finalqry=$empQuery."   order by  ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
        ...
        /*##########*/
        $empRecords = mysqli_query($con, $finalqry);
        ...
        while($row = mysqli_fetch_assoc($empRecords)){
			...
			if($type != ''){
			    $btns = array(
    				...
    				array('edit','gift_quotation.php?res=4&msg=Update Data&id='.$row['id'].'&mod=3','class="btn btn-info btn-xs"  title="Edit"	  '),
    				array('delete','common/delobj.php?obj=quotation&ret=giftquotationList&mod=2&id='.$row['id'],'class="btn btn-info btn-xs griddelbtn" title="Delete" '),
    			);
			}else{
			    $btns = array(
    				//sample
    				//array('delete','quotation_view.php','attrs'),
    				array('view','quotation_view.php','class="show-invoice btn btn-info btn-xs"  title="View Quotation"	data-socode="'.$row['socode'].'" data-st="'.$row["quotationstatusname"].'"  '),
    				array('edit','quotationEntry.php?mode=update&res=4&msg=Update Data&id='.$row['id'].'&mod=3','class="btn btn-info btn-xs"  title="Edit"	  '),
			...
			//disable button if order status is 2//confirmed order;
			if($row['orderstatus'] != 1){
			    $btns[1] = array('edit','#','class="btn btn-info btn-xs" disabled  title="Not Allowed"');
			    $btns[2] = array('delete','#','class="btn btn-info btn-xs" disabled  title="Not Allowed"');
			}
		
			if($row['sl'] >3){
			    $btns[0] = array('view','#','class="btn btn-info btn-xs" disabled  title="Not Allowed"');
            ...
            $data[] = array(
                   ...
                   // "id"=>$i,//$row['hrName'],
					...
					"expted_deliverey_date" => ($row['orderstatus'] == 8)?$row['expted_deliverey_date']:'<div class="deldatewrap"><span>'.$row['expted_deliverey_date'] .'</span><a class="pop-deliverydate" data-socode="'.$row['socode'].'" href="change_delivery_date.php"><i class="fa fa-edit"></i></a></div>',
					
				//	"srctype"=>$row['srctype'],

            	//	"hrName"=>$row['customer'],

            		"organization"=>$row['customer'],
					...
					"orderstatus"=> '<kbd class="orstatus_'.$row['orderstatus'].'">'.$row['quotationstatusname'].'</kbd>',
