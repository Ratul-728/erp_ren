...
$qryInfo = "SELECT iod.frombranch current_branch, iod.qty transfer_stock, i.barcode, i.name productnm, s.freeqty, iod.product 
            ...
            LEFT JOIN chalanstock s ON (s.product=i.id AND s.storerome=iod.frombranch) WHERE io.ioid ='".$orderId."' and iod.product=".$product;
...
while ($rowInfo = $resultInfo->fetch_assoc()) {
	...
	$chkCondArr = array('barcode' => "$barcode",'storerome' => $storeto);
	$chkStkQty = fetchSingleDataByArray('chalanstock',$chkCondArr,'freeqty');

	if($chkStkQty){$action = 'update';}else{$action = 'insert';}
