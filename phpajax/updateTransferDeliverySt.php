...
*/
    ...
    if($col == 'delivered_qty'){
        ...
        $qryInfo = "SELECT tsd.from_store current_branch, tsd.to_store transfer_branch, tsd.qty transfer_stock, i.barcode, i.name productnm, s.freeqty, tsd.product 
            ...
            LEFT JOIN chalanstock s ON (s.product=i.id AND s.storerome=tsd.from_store) WHERE ts.toid ='".$socode."' and tsd.product=".$product;
...
while ($rowInfo = $resultInfo->fetch_assoc()) {
    ...
    if($curstore == 10){
        $qryUpdateStock = "UPDATE `stock` SET `freeqty`= (freeqty + $qty) WHERE product = ".$pid;
