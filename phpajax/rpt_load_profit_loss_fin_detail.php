...
,COALESCE((select  sum(d.amount) from  gldlt d,glmst m where d.vouchno=m.vouchno and d.glac=gl.glno and d.dr_cr='D' and (m.transdt between STR_TO_DATE('$fd', '%d/%m/%Y') and STR_TO_DATE('$td', '%d/%m/%Y')) and m.isfinancial in('0','Y')
 ),0) debit
  ,0 credit
 from 
(
select glno,glnm,opbal from coa_mon where yr='2023' and mn=7 and ctlgl in(select glno from coa_mon where yr='2023' and mn=7 and ctlgl='$gl2') and isposted='P'
 union all 
 select glno,glnm,opbal from coa_mon where yr='2023' and mn=7 and ctlgl='$gl2' and isposted='P' 
 union all 
 select glno,glnm,opbal from coa_mon where yr='2023' and mn=7 and glno='$gl2' and isposted='P') gl  "; 
...
*/
        ...
        { 
            while ($row2inc = $result2inc->fetch_assoc())
            {
                ...
                <?php
                $gl2amti=0;
                $amtqryi="select glno,glnm
,COALESCE((select  sum(d.amount) from  gldlt d,glmst m where d.vouchno=m.vouchno and gl.glno=d.glac and d.dr_cr='D' and (m.transdt between STR_TO_DATE('$fd', '%d/%m/%Y') and STR_TO_DATE('$td', '%d/%m/%Y')) and m.isfinancial in('0','Y')
 ),0) debit
 ,COALESCE((select  sum(d.amount) from  gldlt d,glmst m where d.vouchno=m.vouchno and gl.glno=d.glac and d.dr_cr='C' and (m.transdt between STR_TO_DATE('$fd', '%d/%m/%Y') and STR_TO_DATE('$td', '%d/%m/%Y')) and m.isfinancial in('0','Y')
 ),0) credit
from
(
select glno,glnm from coa where ctlgl='$gl2inc' and isposted='P'
union all 
SELECT glno,glnm from coa where ctlgl in(select glno from coa where ctlgl='$gl2inc' and isposted='P') and isposted='P'
union all
select glno,glnm from coa where ctlgl in (
	SELECT glno from coa where ctlgl in(select glno from coa where ctlgl='$gl2inc' and isposted='P') and isposted='P' ) and isposted='P'
 ) gl ";
                    //echo  $amtqry;die;          
