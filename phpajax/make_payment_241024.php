...
else
{
	...
	else
	{
		...
		if($mode == 2)
		{
		    ...
		    $qry_approval = "INSERT INTO `approval_check`(`invoice`, `checkno`, `checkdt`, `bank`, `amount`, `note`, `image`, `makeby`, `makedt`) 
		                                        VALUES ('$inv_id','$checkno','$checkdate','$bank','$amt','$rem','$image','$hrid',sysdate())";
    ...
    else
    {
		...
		$cmbmode = $data['paywith'];if($cmbmode=='1'){$cashamount=$amt;}else {$cashamount=0;}
        $invsqry="UPDATE `invoice` set `paidamount`=paidamount + $amt ,cashpaid=cashpaid + $cashamount ,`dueamount`=dueamount-$amt,`paymentSt`=$payst,makedt='".date("Y-m-d H:i:s")."' where `invoiceno`='$inv_id'";
        // echo $itqry;die;
        ...
        $orgwallet="insert into organizationwallet(`transdt`,`orgid`,`transmode`,`dr_cr`,`trans_ref`,`amount`,balance,`remarks`,`makeby`,`makedt`)
            values('".date("Y-m-d H:i:s")."',$orgid,'Auto','D','$inv_id',$amt,$curbal,'Paid against invoice',$hrid,'".date("Y-m-d H:i:s")."')";
        //echo $orgwallet;die;
        ...
        /* Accounting*/ 
            ...
            else 
            {
    	         ...
    	         echo json_encode($response);
                   ...
                   //Mail to QA Team
                    /*
                    $qrymail = "SELECT id,active FROM `email` WHERE id = 11";
                    ...
                    $qrymail = "SELECT id,active FROM `email` WHERE id = 9";
                      ...
                      //Mail to Customer
                    $qrymail = "SELECT active FROM `email` WHERE id = 10";
                      $resultmail = $conn->query($qrymail);
                      while($rowmail = $resultmail->fetch_assoc()){
                          $active = $rowmail["active"];
