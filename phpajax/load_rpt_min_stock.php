                        	    ...
                        	    <div class="col-lg-3 col-md-6 col-sm-6">
                                     <div class="form-group">
                                        ...
                                        <label for="cmbsupnm">    Green Taj Center,8/A,Dhanmondi-15, Dhaka 1209. </label><br>
                                         <label for="cmbsupnm">   Minimum  Stock Report </label><br>
...
<?php 
$tcp=0;$tmp=0;
$qry2="SELECT distinct p.id,g.name gn,c.title cn,t.name tn,p.name pn,s.freeqty,s.bookqty,s.costprice,p.mrp,r.name str,p.minimumstock,st.freeqty tqty
FROM chalanstock s ,product p ,catagorygrouping cg,itemgroup g,catagory c, itemtype t ,storeroom r,stock st
where s.product = p.id and p.catagory=cg.itemtype and cg.itemgroup=g.id and cg.itemcatagory=c.id and cg.itemtype=t.id and s.storerome=r.id and st.product=p.id
and s.freeqty<p.minimumstock 
and (s.storerome=".$dagent." or ".$dagent." =0) and (g.id=".$grp." or ".$grp." =0)
...
$result2 = $conn->query($qry2); if ($result2->num_rows > 0) {while($row2 = $result2->fetch_assoc()) 
{
    ...
    $freeqty=$row2["freeqty"]; $cup=$row2["costprice"]; $mup=$row2["mrp"];$tq=$row2["tqty"];$minstk=$row2["minimumstock"];
...
?>                                                  <tr> 
                                                    ...
                                                    <td class="text-right"><?php echo $str?></td>
                                                    ...
                                                    <td><?php echo number_format($tq,0);?></td>
                                                    <td><?php echo number_format($minstk,0);?></td>
                                                </tr>
