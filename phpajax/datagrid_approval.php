...
else if($action=="cancelorder")
{
        ...
        $strwithoutsearchquery="SELECT ro.id,ro.co_id, ro.order_id,  DATE_FORMAT(q.orderdate, '%d/%b/%Y') orderdate,org.id oid, org.name customer,i.name product,qd.qty orderqty,ro.qty_canceled 
                            ...
                            ,ro.st st,qd.otc,qd.discounttot,
        ...
        $sl=0;
        while ($row = mysqli_fetch_assoc($empRecords)) { 
            $rt=$row['discounttot']/$row['orderqty']*$row['qty_canceled'];
...
else if($action=="gift")
{
        ...
        $strwithoutsearchquery="SELECT q.socode, date_format(q.`orderdate`,'%d/%b/%Y') orderdate, org.name customer, i.id invid, i.approval,
                                ...
                                LEFT JOIN hr h1 ON h1.id=i.makeby LEFT JOIN employee emp1 ON emp1.employeecode=h1.emp_id
                                WHERE i.approval IN (3,4,0)  AND i.soid LIKE 'GIFT%' ";
