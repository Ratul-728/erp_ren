                        	    ...
                        	    <div class="col-lg-3 col-md-6 col-sm-6">
                                     <div class="form-group">
                                        ...
                                        <label for="cmbsupnm">   Green Taj Center,8/A,Dhanmondi-15, Dhaka 1209</label><br>
                                         <label for="cmbsupnm">   Store Wise Stock Report </label><br>
...
<?php 
$tcp=0;$tmp=0;
$qry2="SELECT g.name gn,c.title cn,t.name tn,p.name pn,s.freeqty,s.bookqty,s.costprice,p.mrp,r.name str,s.barcode,DATE_FORMAT(s.expirydt,'%e/%c/%Y') expirydt FROM chalanstock s ,product p ,catagorygrouping cg,itemgroup g,catagory c, itemtype t ,storeroom r
where s.product = p.id 
and p.catagory=cg.itemtype and cg.itemgroup=g.id and cg.itemcatagory=c.id and cg.itemtype=t.id and s.storerome=r.id and s.`freeqty`>0
and (s.storerome=".$dagent." or ".$dagent." =0) and (s.barcode='".$bc1."' or '".$bc1."'='') and s.expirydt< STR_TO_DATE('".$td."','%d/%m/%Y')
...
$result2 = $conn->query($qry2); if ($result2->num_rows > 0) {while($row2 = $result2->fetch_assoc()) 
{
    ...
    $freeqty=$row2["freeqty"]; $cup=$row2["costprice"]; $mup=$row2["mrp"]; $bc=$row2["barcode"];$expdt=$row2["expirydt"];
...
?>                                                  <tr> 
                                                    ...
                                                    <td class="text-right"><?php echo $bc?></td>
