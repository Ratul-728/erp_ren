...
if($action == "returnorder")
{
    ...
    if($st == 0)
    {
        $qryUpdate = "UPDATE `return_order` SET `st`='0', approved_by = '$usr', actiondt = sysdate() WHERE id = ".$id;
        if($conn->query($qryUpdate) == true)
        {
            $msg = "Successfully declined!";
        }
    ...
    {
        $qryUpdate = "UPDATE `return_order` SET `st`='2', approved_by = '$usr', actiondt = sysdate() WHERE id = ".$id;
        if($conn->query($qryUpdate) == true)
        {
           $msg = "Successfully accepted!";
            //Get Info
