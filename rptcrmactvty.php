...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['export'])) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C1', 'Activity Maker')
            ->setCellValue('D1', 'ACTIVITY TYPE')
            ->setCellValue('E1', 'CUSTOMER')
            ->setCellValue('F1', 'NOTE')
            ->setCellValue('G1', 'ALOWANCE')
            ->setCellValue('H1', 'AMOUNT')
            ->setCellValue('I1', 'ORGANIZATION');

        $firststyle = 'A2';
        $qry        = "SELECT DATE_FORMAT(d.`comndt`,'%c/%e/%Y %h:%i:%s %p') dt,h.hrName ,t.`name` acttp ,c.`name` cus,d.note,d.place alowance,d.status,d.value ,org.name ornm
FROM `comncdetails` d,`contact` c,`hr` h ,`comnctype` t,organization org
where d.`contactid`=c.`id` and d.`makeby`=h.`id` and d.`comntp`=t.`id` and c.`contacttype`=1 and c.organization=org.orgcode order by d.`comndt` desc";
        // echo  $qry;die;
        ...
        if ($result->num_rows > 0) {$i = 0;
            while ($row = $result->fetch_assoc()) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue($col7, $row['alowance'])
                    ->setCellValue($col8, $row['value'])
                    ->setCellValue($col9, $row['ornm']); /* */
                $laststyle = $title;
            }
        }
                	...
                	<form method="post" action="rptcrmactvty.php" id="form1">
                     ...
                     <div class="well list-top-controls">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>CRM <i class="fa fa-angle-right"></i> All Items </h6>
                       </div>
...
function table_with_filter(url){
	...
	url = 'phpajax/datagrid_crmactivity.php';
	table_with_filter(url);	
        ...
        $('#filter_date_from').daterangepicker({
          ...
          console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
        	...
        	if(start<end){
        	url = 'phpajax/datagrid_crmactivity.php?dt_f='+start.format('YYYY-MM-DD')+'&dt_t='+end.format('YYYY-MM-DD');
        	}
        	else
        	{
        	url = 'phpajax/datagrid_crmactivity.php?dt_f='+end.format('YYYY-MM-DD')+'&dt_t='+start.format('YYYY-MM-DD');
        	}
        	//alert(url);
        	//setTimeout(function(){
        		table_with_filter(url);
		...
		//convert pdf trigger;
			...
			$("#exportpdf").on("click",function(){
				...
				var pdfurl = "pdf_rptcrmactvty.php";
