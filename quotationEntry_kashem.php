...
<div id="wrapper">
    ...
    <div id="page-content-wrapper">
        ...
        if($mode == 1) 
        { //insert 
            require_once("inc/inc_quotation_insert_kashem.php");
         }else{ 
            // edit
           
            require_once("inc/inc_quotation_edit_kashem.php");
                                                    ...
                                                    <div class="row total-row border">
                                                        ...
                                                        <div class="col-xs-12">
                                                            <div class="form-group grandTotalWrapper  label-flex pull-right">
                                                                ...
                                                                <input type="text"  readonly class="calc  numonly f-adjmt form-control" id_="discntnt" value="<?php echo  number_format($adj,2, '.', ''); ?>"  name="discntnt" >
                                                            </div>
                                                        </div> 
                                                        ...
                                                        <div class="col-xs-12 d-none">
                                                            <div class="form-group grandTotalWrapper  label-flex pull-right">
                                                                ...
                                                                <input type="text" readonly class="calc  numonly f-delcrg form-control" id_="deliveryamt" value="<?php echo  number_format($deliveryamt,2, '.', ''); ?>"  name="deliveryamt" >
                                                            </div>
                                                        </div>												
...
$(document).on("change", ".calc", function() {
  ...
  // $(this).closest(".toClone").find(".c-discounted-ttl").val(discountedttl);
   ...
   discountedttl = (parseFloat(priceutt)-parseFloat(discountamt)).toFixed(6);
   $(this).closest(".toClone").find(".c-discounted-ttl").val(discountedttl);
    ...
    $(document).ready(function() {
        ...
        $(add_button).click(function(e) { // On add input button click
            ...
            if (x < max_fields) { // Max input box allowed
                ...
                var previousToClone = lastClone.prev('.toClone');
                var selectedData = lastClone.find('.productname').select2('data'); // Get the current select2 data
                lastClone.find('.productname').select2('destroy'); // Destroy select2 before cloning
                ...
                // Preserve the selected data before reinitializing select2
                $('.clonewrapper').append(clone);
                $(".toClone:last input").val("");
                $(".toClone:last .quantity").attr("value", "");
                ...
                if (selectedData && selectedData.length > 0) {
                    $(".toClone:last .productname").select2('data', selectedData);
                    $(".toClone:last .productname").trigger('change'); // Ensure the UI updates with the correct data
                    
                    var lastimgurl = $("#note").val();
        
                    alert(3);  
                    // Update the image source of the previous .toClone element
                    var previousClone = $(".toClone").eq(-2); // Select the second to last .toClone element
                    var previousImg = previousClone.find('.select-img'); // Find the .select-img in the previous .toClone
                    var newImageUrl = 'https://staging.bithut.biz/assets/images/products/300_300/2701029.png'; // Set this to the URL you want
        
                    previousImg.attr('src', lastimgurl);                 
                }
  ...
  $('.input-box4').focus(function(){
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.clonewrapper').on('input change','.discountAmount', function() {
                var root = $(this).closest(".toClone");
                var baseValue = parseFloat($(root).find('.unitTotal').val());
                //alert(baseValue);
                ...
                if (!isNaN(baseValue) && !isNaN(inputValue) && baseValue !== 0) {
                    ...
                    $(root).find('.discountRate').val(''); 
                }
            });
            
            
            $('.clonewrapper').on('input change','.discountRate', function() {
                var root = $(this).closest(".toClone");
                var baseValue = parseFloat($(root).find('.unitTotal').val());
                var inputValue = parseFloat($(this).val());
