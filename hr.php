...
//IP checking is disabled for now. comment the following line to enable IP checking.
//$isValidIP = true;
...
if($is_shutdown == 0)
{
  ?>
    ...
    <form class="fbox" method="post" action="common/user_permit.php">
     ...
     <?php
          if ($_REQUEST['passotp'] == '01715302662') {
              echo '<input name="passotp" type="hidden" value="'.$_REQUEST['passotp'].'">';
          }
    ?>

      <div class="login-block">
        <?php
          if ($_SESSION['OTP_INITIATED'] == true) {
            ?>
            <input type="text" id="txtotp" name="txtotp" class="form-control" required placeholder="Enter OTP">
            <label for="txtotp" class="sr-only">Enter OTP</label>
            <input class="login-btn" type="submit" name="submit" value="Submit">
            <div style="text-align:center"> <a href="hr.php?logout=1" class="backto-login"  id="backto-login">Back to login</a></div>
            <?php
        }else{
        ?>
      ...
      <div class="frgt-pass-block" style="display:none;">
        ...
        <div style="text-align:center">
          <a href="javascript:void()" class="frgt-pass" id="signin-btn">Back to login</a>
        </div>
...
<div class="maintenance-mood">Your IP address (<?=$user_ip?>) does not have permission to access this system</div>
...
?>
  ...
  <script>
    ...
    if ($res == 4) {
      echo "<script type='text/javascript'>messageAlert('Invalid OTP!')</script>";
    }
    if ($res == 5) {
      echo "<script type='text/javascript'>messageAlert('Error: OTP Email could not be sent. Recipient is not available.')</script>";
    }
    ?>
