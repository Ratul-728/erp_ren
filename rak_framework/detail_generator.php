	...
	foreach($rows as $key => $value)
	{
		...
		if(is_array($value))
		{
				...
				switch ($value['action'])
				{
					...
					case 'fetchByIDVirtual':
					$foundVirtualValue = fetchByID($value['from_table'],$value['find_by'],$value['find_with'],$value['find']);
					if($foundVirtualValue){
							if($value['link_button']=='true'){
									echo '<a href="'.$value['path_to_append'].$foundVirtualValue.'" class="grid_download">Download</a>';
								}else if($value['link_value']=='true'){
									echo '<a href="'.$value['path_to_append'].$foundVirtualValue.'">'.$foundVirtualValue.'</a>';
								}else{
									echo $foundVirtualValue;
								}
						}else{
							echo 'NA';
							}
					...
					if(count($value['dataArray'])>0){
						foreach($value['dataArray'] as $FromArraykey => $FromArrayvalue){
							$dataKeyToList = $FromArrayvalue[$value['dataKeyToList']];
							//echo $dataKeyToList.'<br>';
					...
					case 'download':
					echo '<a href="'.$data[$key].'" class="grid_download" target="_blank">Download</a>';
					break;
					
					case 'downloadWithAuthenticate':
					echo '<form method="post" action="'.$_SERVER['PHP_SELF'].'">';
					echo '<table border="0" cellpadding="5" cellspacing="0"><tr>';
					echo '<td><input type="text" name="'.$value['controlName'].'" placeholder="Enter token no to download the RFP" /></td><td><input id="downloadWithAuthenticateBtn" type="submit" value="Download"  class="grid_download"></td>';
					echo '</tr></table>';
					echo '<input type="hidden" name="postType" value="downloadWithAuthenticate"><input type="hidden" name="click_action" value="detail"><input type="hidden" name="postaction" value="detail"><input type="hidden" name="id" value="'.$value['curDataID'].'">';
					echo '</form>';
					echo '
					<script>
					$("#downloadWithAuthenticateBtn").click(function(){
						
						if($("input[name='.$value['controlName'].']").val()==""){
							alert("Please enter '.$value['controlName'].' first");
							return false;
						}
					});
					</script>
...
//					echo getHTMLBlock($value['url'],$value['start_tag'],$value['start_tag'],$value['end_tag'],$value['show_tags']);
					...
					case 'valueOnTrueFlse':
					if($data[$key] == $value['val_1']){
						echo $value['state1'];
						}else{
						echo $value['state2'];
						}
...
/*				if($value['action'] == 'fetchByID')
		...
		else
		{
			...
			echo '<div class="td">'.nl2br($data[$key]).'</div>';
		}		
