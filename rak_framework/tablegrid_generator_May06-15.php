...
function generateTableGrid($data,$rowPerPage)
{
	...
	global $is_detail, $is_edit, $is_delete, $is_paging,$is_return, $module, $postaction;
	//echo ' - '.$module.' - ';
...
//filter
	...
	if(search_array("fetchByID", $columns) || search_array("filterOnly", $columns)) //DEV
	{
...
//print_r($strlen);
	...
	foreach($columns as $key => $value)
	{
		if(is_array($value))
		{
			...
			else if($value['action'] == 'fetchByID' && $value['asdsorder'])
			{
				...
				echo '</div>';
			}else if($value['action'] == 'customGridIcon' || $value['action'] == 'customCheckBox')
			{
	...
	if($is_detail == 'true' ||	$is_edit == 'true' || $is_delete == 'true' || $value['action'] == 'customGridIcon' || $value['action'] == 'customCheckBox'){	
