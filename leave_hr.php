...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['add'])) {
        header("Location: " . $hostpath . "/add_leave_hr.php?res=0&msg='Insert Data'&mod=4");
    }
    if (isset($_POST['export'])) {
        ...
        $firststyle = 'A2';
        $qry        = "SELECT l.hrid,h1.hrName, des.name desig ,dept.name dept
...
FROM  `leave` l, leaveType lt,hr h ,hr h1,employee e LEFT JOIN department dept ON dept.id=e.department LEFT JOIN designation des ON des.id=e.designation
                ...
                <div >
                    ...
                    <table id='listTable' class='display dataTable actionbtn firstcolpad0' width="100%">
                        ...
                        <tr>
                            ...
                            <th>Documents</th>
                            <th>Reliver</th>
                            <th>Reliver Action</th>
                            <th>Reliver Comments</th>
                            <th>Reliver Approved Date</th>
                            <th>Approver</th>
...
function table_with_filter(url){
             ...
             var table1 = $('#listTable').DataTable({
				...
				'columns': [
                   { data: 'applydt' },
					...
					{ data: 'relinm' },
                    { data: 'reliveraction' },
                    { data: 'relivercomments' },
					{ data: 'releveddate' },
					{ data: 'approver' },
					{ data: 'approveraction' },
					{ data: 'approvercoments' },
					{ data: 'approvedate' },
					{ data: 'action' }
                ]
	...
	url = 'phpajax/datagrid_list_all.php?action=leavehr&empid=<?= $empid ?>';
	table_with_filter(url);	
        ...
        $('#filter_date_from').daterangepicker({
          ...
          console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
        	...
        	if(start<end){
        	url = 'phpajax/datagrid_list_all.php?action=rptleave&empid=<?= $empid ?>&dt_f='+start.format('YYYY-MM-DD')+'&dt_t='+end.format('YYYY-MM-DD');
        	}
        	else
        	{
        	url = 'phpajax/datagrid_list_all.php?action=rptleave&empid=<?= $empid ?>&dt_f='+end.format('YYYY-MM-DD')+'&dt_t='+start.format('YYYY-MM-DD');
        	}
        	//alert(url);
        	//setTimeout(function(){
        		table_with_filter(url);
        ...
        $(".cancelBtn").click(function(){
        	...
        	url = 'phpajax/datagrid_list_all.php?action=rptleave&empid=<?= $empid ?>';
        	table_with_filter(url);
        });
...
$(document).ready(function(){      
  $(".dataTables_wrapper").on("click", ".picture-preview", function() {
    ...
    const fileExtension = mylink.split('.').pop().toLowerCase(); // Get the file extension

    let dialogMessage = '';

    // Check the file type and set the dialog content
    if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(fileExtension)) {
      // If the file is an image
      dialogMessage = `<div id="printableArea4" align="center">
                         <img src="${mylink}" width="100%">
                       </div>`;
    } else if (fileExtension === 'pdf') {
      // If the file is a PDF
      dialogMessage = `<div id="printableArea4" align="center">
                         <iframe src="${mylink}" width="100%" height="600px" style="border:none;"></iframe>
                       </div>`;
    } else {
      // For unsupported file types
      dialogMessage = `<div id="printableArea4" align="center">
                         <p>Preview not available for this file type. <a href="${mylink}" target="_blank">Click here to download/view.</a></p>
                       </div>`;
    }
