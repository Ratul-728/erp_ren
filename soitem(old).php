...
<?php if($mode==1||$mode==5){?> 	            
    ...
    <?php $qryitm="SELECT `id`, `name`, `vat`, `ait`, `cost`  FROM `item`  order by name"; $resultitm = $conn->query($qryitm); if ($resultitm->num_rows > 0) {while($rowitm = $resultitm->fetch_assoc()) 
              { 
                  $tid= $rowitm["id"];  $nm=$rowitm["name"];
                  $cost = number_format($rowitm["cost"],2); $vat = number_format($rowitm["vat"],2); $ait = number_format($rowitm["ait"],2);
    ?>
                                                                <option data-value="<?php echo $tid; ?>" data-cost="<?php echo number_format($cost,2); ?>" data-vat="<?php echo number_format($vat,2); ?>" data-ait="<?php echo number_format($ait,2); ?>" data-value="<?php echo $tid; ?>" value="<?php echo $nm; ?>"><?php echo $nm; ?></option>
    <?php  }}?>                    
                                                            </datalist> 
                                                        </div>
                                                    </div> 
 ...
 <?php $qryunit="SELECT `id`, `name`, `description`, `st` FROM `mu` WHERE st=1  order by name"; $resultunit = $conn->query($qryunit);
							 if ($resultunit->num_rows > 0) {while($rowunit = $resultunit->fetch_assoc()) 
              { 
...
<div class="row header-row">
		...
		<div class="col-lg-1 col-md-6 col-sm-6">
			<h6 class="chalan-head	er">AIT % </h6>
		</div>
		<div class="col-lg-1 col-md-6 col-sm-6">
			<h6 class="chalan-header mgl10"> Select Unit </h6>
		</div>
...
<!--															<lebel>MRC</lebel>-->
                                                            <div class="form-group">
																...
																<input type="hidden" name="unitTotalAmount_mrc" class="unitTotalAmount_mrc" value="0">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div> <!-- this block is for quantity_mrc, unitprice_mrc-->
          	                                    ...
          	                                    <div class="col-lg-2 col-md-6 col-sm-6">
                                                    <div class="form-group">
                                                        <div class="form-group styled-select">
                                                            ...
                                                            <datalist  id="itemName" class="list-itemName form-control">
                                                                <option value="">Select Item</option>
    <?php $qryitm="SELECT `id`, `name`, cost, vat,ait  FROM `item`  order by name"; $resultitm = $conn->query($qryitm); if ($resultitm->num_rows > 0) {while($rowitm = $resultitm->fetch_assoc()) 
              { 
                  $tid= $rowitm["id"];  $nm=$rowitm["name"];
                  $cost = number_format($rowitm["cost"],2); $vat = number_format($rowitm["vat"],2); $ait = number_format($rowitm["ait"],2);
    ?>
...
<?php }} ?>                                     		
                                        ...
                                        <div class="row total-row">
                                            ...
                                            <div class="form-group">
												<table border="0" width="100%" cellpadding="10" cellspacing="20" class="tbl-vatair">
													<tr>
														<td>VAT:</td><td><input type="text" class="form-control" id="totalVat" name="totalVat" value="<?php echo $totalVat;?>" disabled required></td>
														</tr>
														<tr>
														<td>AIT:</td><td><input type="text" class="form-control" id="totalAIT" name="totalAIT" value="<?php echo $totalAIT;?>" disabled required></td>
													</tr>
												</table>
                                        ...
                                        <div class="row total-row">
                                            ...
                                            <div class="form-group grandTotalWrapper">
                                                <label>Total:</label>
                                                <input type="text" class="form-control" id="grandTotalService" value="<?php echo $itdgt;?>" readonly required>
                                              </div>
                                          </div>
                                          </div>
                                      </div>    
...
function CalcServiceOrder(){

  var unittotal;
  var unitTotalAmount_mrc;
  var unitTotalAmount_otc;
	
  
	$(".toClone").each(function(){
		
		//$(this).find(".unitprice_otc, .quantity_otc").change(function() {
		$(document).on("change", ".unitprice_otc,  .quantity_otc", function() {
			
			//alert('OTC');
			
			 qtn_otc	= parseInt($(this).closest('.toClone').find(".quantity_otc").val());
			 up_otc	= parseInt($(this).closest('.toClone').find(".unitprice_otc").val());
			...
			//keep this in hidden value to calculate with total mrg+ total otc togather
			$(this).closest('.toClone').find('.unitTotalAmount_otc').val(utp_otc);
			
			//look for hidden value of unitTotalAmount_mrc amount
			unitTotalAmount_mrc = parseInt($(this).closest('.toClone').find('.unitTotalAmount_mrc').val());
      
			//alert(utp_mrc);
			if(unitTotalAmount_mrc){
        
				unittotal = parseInt(utp_otc)+parseInt(unitTotalAmount_mrc);
				//alert(unittotal);
				$(this).closest('.toClone').find(".unitTotalAmount").val(unittotal);
				}else{
					$(this).closest('.toClone').find(".unitTotalAmount").val(utp_otc);
				}
		...
		//$(this).find(".unitprice_mrc, .quantity_mrc").change(function() {
		$(document).on("change", ".unitprice_mrc, .quantity_mrc", function() {
			//alert('MRC');
			 qtn_mrc	= parseInt($(this).closest('.toClone').find(".quantity_mrc").val());
			 up_mrc	= parseInt($(this).closest('.toClone').find(".unitprice_mrc").val());	
			...
			//save mrc total in hidden field
			$(this).parent().find('.unitTotalAmount_mrc').val(utp_mrc);
			
			//look for hidden value of unitTotalAmount_otc amount
			unitTotalAmount_otc = parseInt($(this).closest('.toClone').find('.unitTotalAmount_otc').val());			
			
      

			if(unitTotalAmount_otc){
				unittotal = parseInt(utp_mrc)+parseInt(unitTotalAmount_otc);
				
        		console.log(unittotal);
				
				$(this).closest('.toClone').find(".unitTotalAmount").val(unittotal);
			}else{
				$(this).closest('.toClone').find(".unitTotalAmount").val(utp_mrc);
			}
...
CalcServiceOrder();		
    ...
    $(".vat").each(function(){
		sumvat += +$(this).val();
	   sumvat1=sumvat.toFixed(2);
     $("#totalVat").val(sumvat1);
  });	
	
	
    $(".ait").each(function(){
		sumait += +$(this).val();
	   sumait1=sumait.toFixed(2);
     $("#totalAIT").val(sumait1);
  });
	*/
	
$(document).on("change", ".unitprice_otc, .unitprice_mrc, .quantity_mrc, .quantity_otc", function() {
    ...
    $(".unitTotalAmount").each(function(){
		...
		//add vat + ait;
		vat = parseInt($("#totalVat").val());
		ait = parseInt($("#totalAIT").val());
		
		sum1 = sum+vat+ait;
		
		sum1=sum1.toFixed(2);
		
     $("#grandTotalService").val(sum1);
  });	
