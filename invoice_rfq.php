...
if ($usr == '') {
    ...
    $qry = "SELECT a.id, a.`poid`,a.`podate`, a.`delivery_address`, a.`note`, a.`st`, org.name, org.contactno, org.email
                                FROM `rfqpo` a LEFT JOIN organization org ON org.id = a.`vendor` 
                                WHERE a.id = ".$invid;
    ...
    if ($resultinv->num_rows > 0) {
        while ($rowinv = $resultinv->fetch_assoc()) {
            ...
            $podt   = $rowinv["podate"];
            $vendor = $rowinv["name"];
            $delivery_address     = $rowinv["delivery_address"];
            $note   = $rowinv["note"];
            $orgcontact   = $rowinv["contactno"];
            $orgemail   = $rowinv["email"];
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1>PO: <?= $poid ?></h1></td>
    </tr>
  </tbody>
</table>
<hr>
  ...
  <tbody>
    <tr>
		...
		<td valign="top">
			...
			<p>
				Date: <?=date_format(date_create($podt),"jS F, Y")?><br>
				Vendor:  <?=$vendor?><br>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-items">
  <tbody>
    <tr class="inv-header">
      ...
      <th scope="col" align="left">Quotation</th>
      <th scope="col">QTY</th>
...
$tot    = 0;
$sl = 0;
$dqry   = "SELECT i.name product, i.image, rv.quotation, rv.`offered_qty`, rv.quated_price 
            FROM `rfqpo_details` rd LEFT JOIN rfq_authorisation ra ON rd.`rfq_auth` = ra.id LEFT JOIN rfq_vendor rv ON rv.id = ra.rfq_vendor 
            LEFT JOIN item i ON i.id = rv.product WHERE rd.`pono` =  '" . $poid . "' ORDER BY i.name asc";
...
if ($resultd->num_rows > 0) {
    while ($rowsd = $resultd->fetch_assoc()) {
		...
		$productPhoto                       = $rowsd["image"];
		$quotation                           = $rowsd["quotation"];
		$qty                           = $rowsd["offered_qty"];
		$unitPrice                           = $rowsd["quated_price"];
		$totamount = $qty*$unitPrice;
		$tot += $totamount;
    
                 
                if ( strlen($productPhoto) > 0){
        		    $photo= $hostpath."/assets/images/products/300_300/".$productPhoto;
        		}else{
    ...
    <tr>
      ...
      <td class="number"><?=number_format($totamount, 2, ".", ",")?></td>
    </tr>
<?php }
//$vat=$tot*0.0;
    //$sc=$tot*0.0;
	...
	<tr>
      <td colspan="4" rowspan="7" class="no-lb-border">&nbsp;</td>
