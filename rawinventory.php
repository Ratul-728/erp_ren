...

{
    ...
    */
    ...
    if ( isset( $_POST['add'] ) ) {
           ...
           header("Location: ".$hostpath."/item.php?res=0&msg='Insert Data'");
    ...
    $qry="SELECT p.`code` `modelCode`,p.`name` `productName`, p.image `prodPhoto`,sum(s.`qty`) qty, avg(s.`cost`) cst, avg(p.`rate`) rate FROM `rawstock` s,`item` p  WHERE s.`itemid`=p.id group by p.`code`,p.name order by p.`code` asc LIMIT ".$limitst. ",".$limitnd;
        ...
        while($row = $result->fetch_assoc()) 
          ...
          { 
                ...
                $seturl="product.php?res=4&msg='Update Data'&id=".$uid;
                  ...
                  $photo="common/upload/item/".$row["modelCode"].".jpg";
