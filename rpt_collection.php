...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['export'])) {
        ...
        $tdt = $_POST["to_dt"];
        if($fdt != ""){
          $date_qry = "AND c.trdt BETWEEN '$fdt' AND '$tdt'";
        }else{
          $date_qry = "";
        }
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E1', 'Mode of Collection');

        $firststyle = 'A2';
        $qry        = "select o.orgcode,o.name orgname,c.trdt,c.amount,m.name modeofpayment
        ...
        if ($result->num_rows > 0) {$i = 0; $optot = 0;
            while ($row = $result->fetch_assoc()) 
            {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue($col1, $i)
                    ->setCellValue($col2, $row['orgcode'])
                    ->setCellValue($col3, $row['orgname'])
                    ->setCellValue($col4, $row['amount'])
                    ->setCellValue($col5, $row['modeofpayment'])
                    ;
            ...
            ->setCellValue($col4, $optot); 
        }
        $objPHPExcel->getActiveSheet()->setTitle('Collection Report');
        $objPHPExcel->setActiveSheetIndex(0);
        $today     = date("YmdHis");
        $fileNm    = "data/" . 'rpt_collection' . $today . '.xls';
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="rpt_collection.php?pg=1&mod=7" id="form1">
                    ...
                    <!--  <div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Accounting <i class="fa fa-angle-right"></i>Collection Report</h6>
                       </div>
                        ...
                        <div class="pull-right grid-panel form-inline">
                            ...
                            <div class="form-group">
                            <input type="hidden" id="pdfsource" url="pdf_rpt_collection.php">
...
include_once 'common_footer.php';
	...
	url = 'phpajax/datagrid_list_all.php?action=rpt_collection';
	table_with_filter(url);	
        ...
        $('#filter_date_from').daterangepicker({
          ...
          console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
        	...
        	if(start<end){
        	url = 'phpajax/datagrid_list_all.php?action=rpt_collection&dt_f='+start.format('DD/MM/YYYY')+'&dt_t='+end.format('DD/MM/YYYY');
        	$('#from_dt').val(start.format('YYYY-MM-DD'));
        	$('#to_dt').val(end.format('YYYY-MM-DD'));
        	}
        	else
        	{
        	url = 'phpajax/datagrid_list_all.php?action=rpt_collection&dt_f='+end.format('DD/MM/YYYY')+'&dt_t='+start.format('DD/MM/YYYY');
        ...
        $(".cancelBtn").click(function(){
        	...
        	url = 'phpajax/datagrid_list_all.php?action=rpt_collection&dt_f=<?php echo $fdt; ?>&dt_f=<?php echo $fdt; ?>';
        	table_with_filter(url);
        });
