...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['add'])) {
        header("Location: " . $hostpath . "/rfq.php?res=0&msg='Insert Data'&mod=14");
    }
    if (isset($_POST['export'])) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'SL.')
            ->setCellValue('B1', 'RFQ NO')
            ->setCellValue('C1', 'RFQ DATE')
            ->setCellValue('D1', 'REQUISITION BY')
            ->setCellValue('E1', 'STATUS');

        $firststyle = 'A2';
        $qry        = "SELECT rfq.id, rfq.rfq, rfq.date,concat(emp.firstname, ' ', emp.lastname) empname, rfq.`st` 
                                FROM `rfq` rfq LEFT JOIN employee emp ON emp.id = rfq.rfq_by";
        // echo  $qry;die;
        ...
        if ($result->num_rows > 0) {$i = 0;
            while ($row = $result->fetch_assoc()) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue($col1, $i)
                    ->setCellValue($col2, $row['rfq'])
                    ->setCellValue($col3, $row['date'])
                    ->setCellValue($col4, $row['empname'])
                    ->setCellValue($col5, $st);
                /* */
                //$laststyle=$title;
            }
        }
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="rfqList.php" id="form1">
                     ...
                     <div class="well list-top-controls">
                      <!--<div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>CRM <i class="fa fa-angle-right"></i>All RFQ</h6>
                       </div>
     ...
     <!-- Datatable JS -->
        ...
        $(document).ready(function(){
            var table1 = $('#listTable').DataTable({
				...
				/*'searching': true,*/
                'ajax': {
                    'url':'phpajax/datagrid_list_all.php?action=rfq'
                },
                'columns': [
                    { data: 'sl' },
                    { data: 'rfq' },
