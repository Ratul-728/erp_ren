        ...
        { $i=0;$tcp=0;$tmp=0;
            ...
            { 
                ...
                $objPHPExcel->setActiveSheetIndex(0)
    						...
    						->setCellValue($col3, $row['itmnm'])
    					    ->setCellValue($col4, $row['itmcode'])
    					    ->setCellValue($col5, $row['itmgrp'])
    					    ->setCellValue($col6, $row["freeqty"])
    					    ->setCellValue($col7, $row["costperunit"])
    					    ->setCellValue($col8, $row["totalcost"]);	/* */
    			$laststyle=$title;	
            }
        }
        
        $objPHPExcel->getActiveSheet()->setTitle('Inventory Value Report ');
        $objPHPExcel->setActiveSheetIndex(0);
        $today=date("YmdHis");
        $fileNm="data/".'inventory_value_'.$today.'.xls'; 
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="#" id="form1" enctype="multipart/form-data">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Inventory <i class="fa fa-angle-right"></i> Inventory Value Report </h6>
                       </div>
...
$qry1    = "SELECT `id`, `name`  FROM `itmCat` order by name";
    ...
    <?php }} ?>
                            ...
                            <div class="form-group">
                            <input type="hidden" id="pdfsource" url="pdf_inventory_value.php">
...
function table_with_filter(url){
	...
	url = 'phpajax/datagrid_report.php?action=inventory_value';
