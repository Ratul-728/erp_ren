...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1>TRANSFER DELIVERY ORDER</h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
<hr>
			<?php
			$qryInfo = "SELECT ts.toid, ts.tansferdt FROM `transfer_stock` ts LEFT JOIN delivery_order dv ON dv.order_id=ts.toid WHERE dv.do_id = '$doid'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-orderinfo">
  <tbody>
    <tr>
		...
		<td valign="top">
			<h5>Transfer Order Informaiton<br></h5>
				Date: <?=date_format(date_create($transferdt),"jS F, Y")?><br>
				Delivery ID: <?=$doid?><br>
				Transfer Order ID:  <?=$transferid?> <br>
