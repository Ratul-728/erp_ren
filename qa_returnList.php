...
else
{
   ...
   if ( isset( $_POST['export'] ) ) {
        ...
        $qry="SELECT qa.order_id,DATE_FORMAT(so.orderdate, '%d/%m/%Y') orderdate, DATE_FORMAT(qa.delivery_date, '%d/%m/%Y %H:%i:%s') delivery_date, COUNT(DISTINCT qa.id) AS noitems, qastatus.name status, qastatus.dclass,inv.invoiceno, 
                    ...
                    WHERE qa.type=3 GROUP BY qa.order_id  order by qa.id desc";
                     ...
                     <div class="well list-top-controls"> 
                      <div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>QA Return <i class="fa fa-angle-right"></i> Return Items </h6>
                       </div>
...
//			}],
    ...
    url = 'phpajax/datagrid_qa.php?action=qareturn';
	table_with_filter(url);	
        ...
        $("#cmbstatus").on("change", function() {
			...
			url = 'phpajax/datagrid_qa.php?action=qareturn&cmbstatus='+status;
