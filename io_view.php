...
if ($usr == '') {
	...
	$qry = "SELECT DATE_FORMAT( io.iodt,'%d/%b/%Y') iodt, iw.name, iw.address from issue_order io LEFT JOIN issue_warehouse iw ON iw.id=io.issue_warehouse where io.ioid = '$ioid'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1>Issue Order</h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
$quotDetailQry ="SELECT tsd.qty, frmbr.name frombranch, i.name product,i.description description,i.image,i.barcode,i.dimension,tsd.`approval_qty`
                FROM `issue_order` ts LEFT JOIN issue_order_details tsd ON ts.id=tsd.ioid LEFT JOIN branch frmbr ON tsd.frombranch=frmbr.id 
