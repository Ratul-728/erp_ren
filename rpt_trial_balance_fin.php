...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
     ...
     */
    $currSection = 'rpt_trial_balance_fin';
    ...
    if (isset($_POST['export'])) {
            ...
            while ($row = $result->fetch_assoc()) 
            {
               ...
               /* if ($row["dr_cr"] == 'C') {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue($col6, $row['op'])
                    ->setCellValue($col7, $row['dr'])
                    ->setCellValue($col8, $row['cr'])
                    ->setCellValue($col9, $row['op']+$row['dr']-$row['cr'])
                    ;
        ...
        $objPHPExcel->setActiveSheetIndex(0);
        $today     = date("YmdHis");
        $fileNm    = "data/" . 'rpt_trial_balance_f' . $today . '.xls';
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="rpt_trial_balance_fin.php?pg=1&mod=17" id="form1">
...
include_once 'common_footer.php';
	...
	url = 'phpajax/datagrid_list_all.php?action=rpt_trial_balance_fin&dt_f=<?php echo $fdt; ?>';
	table_with_filter(url);	
        ...
        $('#filter_date_from').daterangepicker({
          ...
          console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
        	...
        	if(start<end){
        	url = 'phpajax/datagrid_list_all.php?action=rpt_trial_balance_fin&dt_f='+start.format('DD/MM/YYYY')+'&dt_t='+end.format('DD/MM/YYYY');
        	$('#from_dt').val(start.format('YYYY-MM-DD'));
        	$('#to_dt').val(end.format('YYYY-MM-DD'));
        	}
        	else
        	{
        	url = 'phpajax/datagrid_list_all.php?action=rpt_trial_balance_fin&dt_f='+end.format('DD/MM/YYYY')+'&dt_t='+start.format('DD/MM/YYYY');
		...
		//convert pdf trigger;
			...
			$("#exportpdf").on("click",function(){
				...
				var pdfurl = 'pdf_trial_balance_report_fin.php?dt_f='+sdt+'&dt_t='+edt;
