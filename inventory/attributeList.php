...
else
{
    ...
    */
    ...
    if ( isset( $_POST['add'] ) ) {
           header("Location: ".$hostpath."/attribute.php?res=0&msg='Insert Data'&mod=1");
    }
   ...
   from attribute a left join attributevalue v on a.code=v.attribute  GROUP BY a.id,a.name,v.attribute  order by a.name,v.attribute "; 
    ...
    <br>
                	<form method="post" action="attributeList.php" id="form1">
     ...
     <!-- Datatable JS -->       
        ...
        $(document).ready(function(){
            $('#listTable').DataTable({
				...
				scroller: true,
                'ajax': {
                    'url':'phpajax/datagrid_list_all.php?action=attr'
                },
                'columns': [
                    { data: 'name' },
                    { data: 'atv' },
