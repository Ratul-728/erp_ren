...
else
{
   ...
   if ( isset( $_POST['export'] ) ) {
        ...
        $qry="SELECT qa.order_id,DATE_FORMAT(so.orderdate, '%d/%m/%Y') orderdate, DATE_FORMAT(qa.delivery_date, '%d/%m/%Y %H:%i:%s') delivery_date, COUNT(DISTINCT qa.id) AS noitems, qastatus.name status, qastatus.dclass,inv.invoiceno, 
                    ...
                    WHERE qa.type=1  GROUP BY qa.order_id  order by qa.id desc";
                     ...
                     <div class="well list-top-controls"> 
                      <div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Quality <i class="fa fa-angle-right"></i> Issue Order</h6>
                       </div>
...
//			}],
				...
				'columns': [
                    { data: 'sl' },
                    { data: 'ioid', 'orderable': false },
                    { data: 'product' },
                    { data: 'address', 'orderable': false },
                    { data: 'deliverydt', 'orderable': false},
					{ data: 'cbranch' },
                	{ data: 'tbranch', 'orderable': false  },	
            		{ data: 'current_stock', 'orderable': false  },
            		{ data: 'transfer_stock', 'orderable': false  },
    ...
    url = 'phpajax/datagrid_qa.php?action=qaissue';
	table_with_filter(url);	
        ...
        $("#cmbstatus, .dl-itemName").on("change", function() {
			...
			url = 'phpajax/datagrid_qa.php?action=qaissue&customer='+dataValue+'&cmbstatus='+status;
