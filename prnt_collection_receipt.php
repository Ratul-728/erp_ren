...
$qry = "SELECT cl.`id`, cl.`trdt`, tr.name `transmode`,cl.`transref`,c.name `customer`, c.street `address`, cl.`naration`,format(cl.`amount`,2)amount, cc.name `costcenter` , cl.`chequedt`
	...
	$qry = "SELECT c.`id`,DATE_FORMAT(c.trdt,'%e/%c/%Y')  `trdt`, c.`transmode`, c.`transref`, DATE_FORMAT(c.chequedt,'%e/%c/%Y') `chequedt`, c.`customerOrg`, c.`naration`, c.`amount`, c.`costcenter`, c.`chqclearst`, c.`cleardt`, c.`st`, c.`makeby`, c.`makedt`,c.`invoice`,c.currencycode, o.name cname, inv.invoiceNo FROM `collection` c left join organization o ON o.id = c.customerOrg left join invoice inv ON c.invoice = inv.id  where c.id= " . $rpid;
	*/
	$qry = "SELECT c.`id`,DATE_FORMAT(c.trdt,'%e/%c/%Y')  `trdt`, c.`transmode`,t.name trmode, c.`transref`, DATE_FORMAT(c.chequedt,'%e/%c/%Y') `chequedt`, c.`customerOrg`, c.`naration`, c.`amount`, c.`costcenter`, c.`chqclearst`, c.`cleardt`, c.`st`, c.`makeby`, c.`makedt`,c.`invoice`,c.currencycode, o.name cname, inv.invoiceNo FROM `collection` c left join organization o ON o.id = c.customerOrg left join invoice inv ON c.invoice = inv.id 
left join transmode t on c.transmode=t.id where c.id=" . $rpid;
...
if ($conn->connect_error) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $customer = ($row['cname'])?$row['cname']:'NA';
	...
	<table class="tbl-body2" width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
		  <td width="33%"  class="pr-1">
			  ...
			  <div  class="txt-value-b"><?=$trmode?></div>
				<hr class="light">
