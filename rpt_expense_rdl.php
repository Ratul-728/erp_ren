...
order by c.`glno`,m.yr,m.mn;
   ...
   // $fdt = $_POST['fdt'];if ($fdt == '') {$fdt = date('d/m/Y', strtotime('-1 month'));}
    ...
    //echo $vlvl;die;
    //}
    $ajxurl = "phpajax/datagrid_list_all.php?action=rpt_expense_rdl&gllvl='.$vlvl.'&ctrgl=".$vctgl;
    //echo $ajxurl;
...
function table_with_filter(url){
	...
	var yr=$("#cmbyr").val();
//	alert(lvl);
	url = 'phpajax/datagrid_list_all.php?action=rpt_expense_rdl&gllvl='+plvl+'&ctrgl='+pctrl+'&pyr='+yr;
	//	alert(url);
	table_with_filter(url);	
        ...
        $(".cancelBtn").click(function(){
        	...
        	url = 'phpajax/datagrid_list_all.php?action=rpt_expense_rdl';
        	table_with_filter(url);
        });
    ...
    <script>
		//convert pdf trigger;
			...
			$("#exportpdf").on("click",function(){
				...
				var pdfurl = 'pdf_expense_rdl.php?gllvl='+plvl+'&ctrgl='+pctrl+'&pyr='+yr;
