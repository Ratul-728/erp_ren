...
else
{
    ...
    {
        $qry="SELECT `id`, `contactcode`,  `contacttype`, `name`, `organization`, `dob`, `designation`, `department`, `hidden`, `phone`, `email`, `photo`,`location`, `website`, `source`, `sourcename`, `details`, `area`, `street`, `district`, `state`, `zip`, `country`, `opendt`, `status`, `makeby`, `makedt` FROM `contact`  where id= ".$aid; 
       // echo $qry; die;
        ...
        else
        {
            ...
            if ($result->num_rows > 0)
            {
                ...
                { 
                    $acid=$row["id"];$contactcode=$row["contactcode"];$contacttype=$row["contacttype"];  $name=$row["name"];$organization=$row["organization"];  $dob=$row["dob"];
                    ...
                    $state=$row["state"];$zip=$row["zip"]; $country=$row["country"];$website=$row["website"]; $hidden = $row["hidden"];
                }
            }
        }
        $mode=2;//update mode
    ...
    {
        $acid='';$contactcode='';$contacttype='';  $name='';$organization='';  $dob=''; $designation='';  $department='';$phone='';  $email='';$photo='';  $location='';
        $source='';  $sourcename='';$details='';  $area='';$street='';  $district=''; $state=''; $zip=''; $country='';$website='';
        $mode=1;//Insert mode
    }
...
<?php  include_once('common_header.php');?>
    <body class="form">
       ...
       <!-- Page Content -->
                ...
                <div class="container-fluid pagetop">
                    <div class="row">
                        <div class="col-lg-12">
                            <p>&nbsp;</p> <p>&nbsp;</p>
                            <p>
                                <form method="post" action="common/addcontact.php"  id="form1" enctype="multipart/form-data">  <!--onsubmit="javascript:return WebForm_OnSubmit();" -->
        ...
        <?php       //check file exist;
        if(file_exists("common/upload/contact/$photo.jpg")){$photoFilePath = "common/upload/contact/$photo.jpg";}else{$photoFilePath = "images/blankuserimage.png";	}  
        ?>
        									    <div class="col-sm-12"> 
        									        <br>
                                                    <div class="well">
                                                        <div class="row">
                                                            <div class="col-sm-12">
                                                                <div class="side-by-side">
                                                                    <div> <img src="<?=$photoFilePath?>" width="100" height="100" id="pic1" style="margin-bottom:10px;"> </div>
                                                                    <div> 
                                                            <!--strong>Photo</strong-->
                                                                        <div class="input-group">
                                                                            <label class="input-group-btn">
                                                                                 <span class="btn btn-primary btn-file btn-file"> <i class="fa fa-upload"></i>
                                                                                    <input type="file" accept="image/*"  capture="camera" name="attachment1" id="attachment1" style="display: none;" >
                                                                                </span>
                                                                            </label>
        ...
        <?php $qrycntp="SELECT `id`, `name` FROM `contacttype` order by name"; $resultcntp = $conn->query($qrycntp); if ($resultcntp->num_rows > 0) {while($rowcntp = $resultcntp->fetch_assoc()) 
        ...
        <?php $qryorg="SELECT `orgcode` `id`, `name` FROM `organization` order by name"; $resultorg = $conn->query($qryorg); if ($resultorg->num_rows > 0) {while($roworg = $resultorg->fetch_assoc()) 
        ...
        <?php $qrydsg="SELECT `id`, `name` FROM `designation` order by name"; $resultdsg = $conn->query($qrydsg); if ($resultdsg->num_rows > 0) {while($rowdsg = $resultdsg->fetch_assoc()) 
          { 
              $did= $rowdsg["id"];  $dnm=$rowdsg["name"];
        ?>
                                                                <option value="<?php echo $did; ?>" <?php if ($designation == $did) { echo "selected"; } ?>><?php echo $dnm; ?></option>
        <?php  }}?>                                                       
                                                            </select>
                                                        </div>
                                                    </div>         
        ...
        <?php  }}?>                                                       
                                                ...
                                                <div class="col-lg-3 col-md-6 col-sm-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" i d="descr" name="descr" value="<?php echo $details;?>">
                                                    </div>        
                                                </div>
                                                <div class="col-lg-3 col-md-6 col-sm-6">
                                                    <div class="form-group">
...
-->
                                    ...
                                    <div class="button-bar">
                                        <?php if($mode==2) { ?>
            	                        <input  dat a-to="pagetop" class="btn btn-lg btn-default top" type="submit" name="update" value="Update contact"  id="update" > <!-- onclick="javascript:messageAlert('Event is fired from the page and messaged is pased via parameter. will be desolve in 5 sec')" -->
                                        <?php } else {?>
                                        <input  dat a-to="pagetop" class="btn btn-lg btn-default top" type="submit" name="add" value="Add contact"  id="add" >
                                        <?php } ?>
                                        <a href = "./contactList.php?pg=1&mod=2">
