...
else
{
    ...
    $qry = "SELECT org.name orgnm, date_format(q.orderdate,'%d/%b/%Y') orderdate, r.order_id,r.ro_id, d.do_id, rp.delivery_start,d.id
    ...
    <span class="alertmsg">
        ...
        <link href='js/plugins/datagrid/datatables.css' rel='stylesheet' type='text/css'>                    
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Delivery  <i class="fa fa-angle-right"></i> Return Order Delivery Status <i class="fa fa-angle-right"></i> Delivery ID: <?= $deliveryId ?> </h6>
                       </div>
                                ...
                                <tr>
                                    ...
                                    <?php 
                                        $qryQaw = "SELECT  dod.do_qty,dod.id, dod.st, dod.pending_qty, dod.intransit_qty, dod.delivered_qty, dod.due_return_qty,
                                                    dod.returned_qty, b.name warehouse
                                                    FROM `delivery_order_detail` dod LEFT JOIN qa_warehouse qaw ON dod.qa_id=qaw.id 													LEFT JOIN branch b ON qaw.warehouse_id=b.id
...
$(".plus_intransit, .plus_delivery").on("click",function(){
  ...
  if(isChanged){
  $.ajax({
        url:"phpajax/updateRODeliverySt.php",
        method:"POST",
