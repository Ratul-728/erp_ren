   ...
   if ( isset( $_POST['export'] ) ) {
      // print_r($_POST);die;
        ...
        */
        ...
        $qry="select '' VouchNo,DATE_FORMAT(STR_TO_DATE('".$fdt."', '%d/%m/%Y'), '%m/%d/%Y') AS TransDt,'' refno,'Opening Balance' remarks,'' sl,'' glac,'' glnm,$d_bal D_amount, $c_bal C_amount
                   union all
                   select a.VouchNo,DATE_FORMAT( a.TransDt,'%m/%d/%Y') TransDt ,a.refno,a.remarks,d.sl,d.glac,g.glnm,COALESCE((case d. dr_cr when 'D' then d.amount else 0 End),0) D_amount,COALESCE((case d.dr_cr when 'C' then d.amount else 0 End),0) C_amount  
