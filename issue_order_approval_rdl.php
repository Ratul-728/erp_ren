...
if ($usr == '') {
	...
	$qry = "SELECT DATE_FORMAT( ts.iodt,'%d/%b/%Y') iodt,DATE_FORMAT( ts.deliverydt,'%d/%b/%Y') deliverydt, concat(emp.firstname, ' ', emp.lastname) empnm, ts.st, iw.name, iw.address, ts.id 
    ...
    if ($resultinv->num_rows > 0) {
        while ($rowUpperInfo = $resultinv->fetch_assoc()) {

                $ioid = $rowUpperInfo["id"];
        	    $iodt = $rowUpperInfo["iodt"];
        	    $deliverydt = $rowUpperInfo["deliverydt"];
  ...
  <tbody>
    <tr>
		...
		<td valign="top" align="center">
			...
			<p>
				Issue Date: <?= $iodt ?><br>
				Expected Delivery Date: <?= $deliverydt ?><br>
				Name:  <?=$name?><br>
				Address:  <?=$address?><br>
				Request By:  <?=$empnm?><br>
			</p>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-items">
  <tbody>
    <tr class="inv-header">
      ...
      <th scope="col" align="left">From Store</th>
...
$quotDetailQry ="SELECT i.name item, i.barcode, i.image, iod.qty, b.name branch
                FROM `issue_order_details` iod LEFT JOIN item i ON iod.product=i.id LEFT JOIN branch b ON iod.frombranch=b.id
                WHERE iod.ioid = '" . $ioid. "'";	
...
if ($resultd->num_rows > 0) {
    while ($rowsd = $resultd->fetch_assoc()) {
		...
		$sosl++;
		$productcode                     = $rowsd["barcode"];
        $pnm                             = $rowsd["item"];
        $qty                             = $rowsd["qty"];
        $image                           = $rowsd["image"];
        $frombranch                      = $rowsd["branch"];
