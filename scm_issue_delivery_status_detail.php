...
else
{
    ...
    $qryInfo="SELECT ioo.iodt orderdate,iw.name warehouse, iw.address, del.id, rp.delivery_start, ioo.ioid 
            FROM delivery_order del LEFT JOIN `qa` q ON del.order_id = q.order_id LEFT JOIN issue_order ioo ON q.order_id=ioo.ioid 
            LEFT JOIN issue_warehouse iw ON iw.id = ioo.issue_warehouse LEFT JOIN `resourceplan` rp ON del.do_id = rp.doid WHERE del.do_id = '".$deliveryId."'";
    // echo $qryInfo;die;
    $resultInfo = $conn->query($qryInfo);
    while ($rowinfo = $resultInfo->fetch_assoc()) {
        $issuewarehouse = $rowinfo["warehouse"];
    ...
    <div id="wrapper"> 
      ...
      <div class="section">
        ...
        <span>Issue Delivery Status</span>
      </div>
                                ...
                                <div class="row">
                                   <div class="col-md-6">
                                           <b>Delivery ID: </b> <?= $deliveryId ?> <br>
                                            <b>ISSUE ORDER:   </b>  <?= $issueorder ?>
                                    </div>
                                    <div class="col-md-6">
                                            <b>ISSUE WAREHOUSE: </b> <?= $issuewarehouse ?> <br>
                                            <b>Address:   </b> <?= $address ?>
                                    </div>
                               </div>
...
$(".plus_intransit, .plus_delivery").on("click",function(){
  ...
  if(isChanged){
  $.ajax({
        url:"phpajax/updateIssueDeliverySt.php",
        method:"POST",
