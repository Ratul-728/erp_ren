...
if ($usr == '') {
	...
	$qry = "SELECT g.`vouchno`, DATE_FORMAT(g.`transdt`,'%d/%b/%Y') trdt, g.`refno`,g.remarks,h.hrName entryusr,h1.hrName checkusr,h2.hrName aprvusr FROM `glmst` g left join hr h on g.entryby=h.id left join hr h1 on g.checkby=h1.id 
	left join hr h2 on g.approvedby=h2.id	WHERE vouchno = '".$vouchno."'";
    ...
    if ($resultinv->num_rows > 0) {
        while ($rowinv = $resultinv->fetch_assoc()) {
            ...
            $ref   = $rowinv["refno"];
             $narr   = $rowinv["remarks"];
             $entryby   = $rowinv["entryusr"];
			 $checkby= $rowinv["checkusr"];
			 $apprvby=$rowinv["aprvusr"];
        }
    }
}
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1>GL Voucher</h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-items">
  <tbody>
    <tr class="inv-header">
      ...
      <th scope="col" align="left">GL Account</th>
      <th scope="col" class="number">Transaction Type</th>
...
$quotDetailQry ="SELECT gt.`dr_cr`, gt.`amount`, c.glnm FROM `gldlt` gt LEFT JOIN coa c ON c.glno=gt.glac WHERE gt.`vouchno` =  '" . $vouchno. "' ORDER BY c.glnm ASC";	
	  
//echo $quotDetailQry;die;
$totcr = 0; $totdr = 0; $sosl = 0;
...
if ($resultd->num_rows > 0) {
    ...
    <tr>
      ...
      <td class="number"><?=$tnm?></td>
      <td class="number"><?=number_format($amount, 2, ".", ",")?></td>
    </tr>
<?php }} ?>
	<tr>
      <td colspan="2" rowspan="6" class="no-lb-border">&nbsp;</td>
      <th class="itemtbl-footer">TOTAL DEBIT</th>
      <td class="number strong"><?php echo number_format($totdr, 2, ".", ",") ?></td>
    </tr>
    <tr>
      <th class="itemtbl-footer">TOTAL CREDIT</th>
      <td class="number" style="font-size:11px;">(<?php echo number_format($totcr, 2, ".", ","); ?>)</td>
    </tr>
    <tr>
      <th class="itemtbl-footer">NET</th>
      <td class="number" style="font-size:11px;color: #c0c0c0">(<?php echo number_format(($totdr-$totcr), 2, ".", ","); ?>)</td>
    </tr>
...
<div class="terms-wrapper">	
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-item-detail">
        <tr>
          <td align="center" class="no-lb-border"><h2><?php echo $narr;?></h2></td>
        </tr>
    </table>
    <br/><br/>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-item-detail">
        <tr>
          <td align="center" class="no-lb-border"><h2><?php echo $entryby;?></h2></td>
          <td align="center" class="no-lb-border"><h2><?php echo $checkby;?></h2></td>
          <td align="center" class="no-lb-border"><h2><?php echo $apprvby;?></h2></td>
        </tr>
         <tr>
          <td align="center" class="no-lb-border"><h2>----------------------</h2></td>
          <td align="center" class="no-lb-border"><h2>----------------------</h2></td>
          <td align="center" class="no-lb-border"><h2>----------------------</h2></td>
        </tr>
         <tr>
          <td align="center" class="no-lb-border"><h2>Prepared By</h2></td>
          <td align="center" class="no-lb-border"><h2>Checked By</h2></td>
          <td align="center" class="no-lb-border"><h2>Approved By</h2></td>
        </tr>
    </table>
