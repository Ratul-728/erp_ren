...
<?php
$dqry   = "SELECT i.code, i.name product, i.dimension, i.image, i.description,i.parts,
SUM(qaw.ordered_qty) ordered_qty, SUM(qaw.pass_qty) pass_qty, SUM(qaw.defect_qty) defect_qty, SUM(qaw.damaged_qty) damaged_qty
            FROM `qa` q LEFT JOIN qa_warehouse qaw ON qaw.qa_id=q.id LEFT JOIN item i ON i.id = q.product_id 
            WHERE q.type = 1 AND q.order_id = '".$socode."'
...
if ($resultd->num_rows > 0) {
    while ($rowsd = $resultd->fetch_assoc()) {
        ...
        $pass_qty                       = $rowsd["pass_qty"];
        $defect_qty                       = $rowsd["defect_qty"];
        $damaged_qty                       = $rowsd["damaged_qty"];
    ...
    <tr>
      ...
      <td class="number"><?= $pass_qty?></td>
      <td class="number"><?= $defect_qty?></td>
      <td class="number"><?= $damaged_qty?></td>
<?php }} ?>
    </tr>
  </tbody>
</table>
