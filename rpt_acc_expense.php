...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['export'])) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'SL.')
            ->setCellValue('B1', 'Expense Nature')
            ->setCellValue('C1', 'Amount')
            ->setCellValue('D1', 'Mode of Payment');
        ...
        if ($result->num_rows > 0) {$i = 0; $optot = 0;
            while ($row = $result->fetch_assoc()) 
            {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue($col1, $i)
                    ->setCellValue($col2, $row['exp_ntre'])
                    ->setCellValue($col3, $row['amount'])
                    ->setCellValue($col4, $row['modeofpayment'])
                    ;
            ...
            $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue($col2, 'Total')
            ->setCellValue($col3, $optot); 
        }
        ...
        function table_with_filter(url){
             ...
             var table1 = $('#listTable').DataTable({
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    var columnsToTotal = [2]; // Indexes of the columns to total
