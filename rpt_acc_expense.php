...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['export'])) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'SL.')
            ->setCellValue('B1', 'Expense Date')
            ->setCellValue('C1', 'Expense Nature')
            ->setCellValue('D1', 'Debit/Credit')
            ->setCellValue('E1', 'Amount')
            ->setCellValue('F1', 'Mode of Payment');
        ...
        if ($result->num_rows > 0) {$i = 0; $optot = 0;
            while ($row = $result->fetch_assoc()) 
            {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue($col1, $i)
                    ->setCellValue($col2, $row['expenseDate'])
                    ->setCellValue($col3, $row['expnsNature'])
                    ->setCellValue($col4, $row['dr_cr'])
                    ->setCellValue($col5, $row['amount'])
                    ->setCellValue($col6, $row['paymode'])
                    ;
            ...
            ->setCellValue($col5, $optot); 
        }
        $objPHPExcel->getActiveSheet()->setTitle('Expense Report');
        $objPHPExcel->setActiveSheetIndex(0);
        $today     = date("YmdHis");
        $fileNm    = "data/" . 'rpt_acc_expense' . $today . '.xls';
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="rpt_acc_expense.php?pg=1&mod=7" id="form1">
                    ...
                    <!--  <div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Accounting <i class="fa fa-angle-right"></i>Expense Report</h6>
                       </div>
                        ...
                        <div class="pull-right grid-panel form-inline">
                            ...
                            <div class="form-group">
                            <input type="hidden" id="pdfsource" url="pdf_rpt_acc_expense.php">
...
include_once 'common_footer.php';
        ...
        function table_with_filter(url){
             ...
             var table1 = $('#listTable').DataTable({
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    var columnsToTotal = [4]; // Indexes of the columns to total
	...
	url = 'phpajax/datagrid_list_all.php?action=rpt_acc_expense';
	table_with_filter(url);	
        ...
        $('#filter_date_from').daterangepicker({
          ...
          console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
        	...
        	if(start<end){
        	url = 'phpajax/datagrid_list_all.php?action=rpt_acc_expense&dt_f='+start.format('DD/MM/YYYY')+'&dt_t='+end.format('DD/MM/YYYY');
        	$('#from_dt').val(start.format('YYYY-MM-DD'));
        	$('#to_dt').val(end.format('YYYY-MM-DD'));
        	}
        	else
        	{
        	url = 'phpajax/datagrid_list_all.php?action=rpt_acc_expense&dt_f='+end.format('DD/MM/YYYY')+'&dt_t='+start.format('DD/MM/YYYY');
        ...
        $(".cancelBtn").click(function(){
        	...
        	url = 'phpajax/datagrid_list_all.php?action=rpt_acc_expense&dt_f=<?php echo $fdt; ?>&dt_f=<?php echo $fdt; ?>';
        	table_with_filter(url);
        });
