...
$(document).ready(function(){
		...
		function table_with_filter(url){			
           ...
           var table1 = $('#listTable').DataTable({
			   ...
			   "order": [[ 0, "desc" ]],
				"dom": "rtiplf",
                'ajax': {
                    //'url':'phpajax/datagrid_service.php?action=invoice&filterorg=<?=$filterorg ?>&filterst=<?=$filterst ?>&fdt=<?=$fdt ?>&tdt=<?=$tdt ?>&yeardt=<?=$yeardt ?>'
	...
	url = 'phpajax/datagrid_service.php?action=invoice&filterorg=<?=$filterorg ?>&filterst=<?=$filterst ?>&fdt=<?=$fdt ?>&tdt=<?=$tdt ?>&yeardt=<?=$yeardt ?>';
	//url = 'phpajax/datagrid_service.php?action=invoice';
	table_with_filter(url);		
	

        $("#filterst").on("change", function() {
            var filterst = $(this).val(); // alert(filterst);
			url = 'phpajax/datagrid_service.php?action=invoice&filterst='+filterst;
            setTimeout(function(){
				table_with_filter(url);
            }, 350);			
        });		
...
$('#filter_date_from').daterangepicker({
  ...
  console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
	...
	if(start<end){
	url = 'phpajax/datagrid_service.php?action=invoice&dt_f='+start.format('YYYY-MM-DD')+'&dt_t='+end.format('YYYY-MM-DD');
	}
	else
	{
	url = 'phpajax/datagrid_service.php?action=invoice&dt_f='+end.format('YYYY-MM-DD')+'&dt_t='+start.format('YYYY-MM-DD');
	}
	//alert(url);
	//setTimeout(function(){
		table_with_filter(url);
