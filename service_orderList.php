...
else
{
    ...
    */
    ...
    if ( isset( $_POST['add'] ) ) {
           ...
           header("Location: ".$hostpath."/service_order.php?res=0&msg='Insert Data'&mod=22");
    }
   ...
   if ( isset( $_POST['export'] ) ) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A1', 'SL.')
                ->setCellValue('B1', 'SERVICE ORDER')
                ->setCellValue('C1', 'ORDER DATE')
                ->setCellValue('D1', 'CUSTOMER')
                ->setCellValue('E1', 'TOTAL AMOUNT')
                ->setCellValue('F1', 'TOTAL VAT')
                ->setCellValue('G1', 'TRANSPORT COST')
                ->setCellValue('H1', 'SERVICE CHARGE');
                  
    			
        $firststyle='A2';
        $qry="SELECT so.code,DATE_FORMAT(so.orderdate,'%d/%b/%Y') orderdate,so.totalamount,so.totalvat,so.totaltax,so.transport,so.service_charge, org.name, so.id 
       ...
       // echo  $qry;die;
        ...
        { $i=0;
            ...
            { 
                ...
                $objPHPExcel->setActiveSheetIndex(0)
    			            ...
    			            ->setCellValue($col3, $row['orderdate'])
    			            ->setCellValue($col4, $row['name'])
    			            ->setCellValue($col5, $row['totalamount'])
    			            ->setCellValue($col6, $row['totalvat'])
    			            ->setCellValue($col7, $row['transport'])
    			            ->setCellValue($col8, $row['service_charge']);
        ...
        $objPHPExcel->setActiveSheetIndex(0);
        $today=date("YmdHis");
        $fileNm="data/".'service_item'.$today.'.xls'; 
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="service_orderList.php" id="form1">
            
                     <div class="well list-top-controls"> 
                      <div class="row border">
                       
                     <div class="col-sm-3 text-nowrap">
                            <h6>Service <i class="fa fa-angle-right"></i>All Service Order</h6>
                       </div>
     ...
     <!-- Datatable JS -->       
        ...
        $(document).ready(function(){
            var table1 = $('#listTable').DataTable({
				...
				/*'searching': true,*/
                'ajax': {
                    'url':'phpajax/datagrid_service.php?action=serviceorder'
                },
                'columns': [
                    { data: 'id' },
                    { data: 'code' },
...
//show INVOICE
   ...
   //alert(mylink);
  	...
  	BootstrapDialog.show({
							...
							title: 'Service Invoice #'+$(this).data('code'),
