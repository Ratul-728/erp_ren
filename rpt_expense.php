...
order by c.`glno`,m.yr,m.mn;
    */
    ...
    if (isset($_POST['export'])) {
        ...
        $firststyle = 'A2';
        $qry        = "SELECT  e.id,date_format(e.trdt,'%m/%d/%Y') trdt,t.name transmode,p.name transtype,e.amount,e.naration 
        ...
        if ($result->num_rows > 0) {$i = 0;
            while ($row = $result->fetch_assoc()) {
                ...
                $dateValue = PHPExcel_Shared_Date::PHPToExcel(strtotime($row['trdt']));
