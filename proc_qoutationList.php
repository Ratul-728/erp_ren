...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['export'])) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E1', 'Validity DATE');

        $firststyle = 'A2';
        $qry        = "SELECT a.id, a.`rfq`, a.`date`, concat(emp.firstname, ' ', emp.lastname) empname, a.`validity_date` 
                                FROM `rfq` a LEFT JOIN employee emp ON a.`rfq_by` = emp.id 
                                WHERE a.`st` = 1 order by a.id desc";
        // echo  $qry;die;
        ...
        if ($result->num_rows > 0) {$i = 0;
            while ($row = $result->fetch_assoc()) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue($col5, $row['validity_date']);
                /* */
                //$laststyle=$title;
            }
        }
        ...
        $objPHPExcel->setActiveSheetIndex(0);
        $today     = date("YmdHis");
        $fileNm    = "data/" . 'quotation' . $today . '.xls';
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="proc_qoutationList.php" id="form1">
                     ...
                     <div class="well list-top-controls">
                      <!--<div class="row border">
                       ...
                       <div class="col-sm-3 text-nowrap">
                            <h6>Procurement <i class="fa fa-angle-right"></i>All Qoutation</h6>
                       </div>
     ...
     <!-- Datatable JS -->
        ...
        $(document).ready(function(){
            var table1 = $('#listTable').DataTable({
				...
				/*'searching': true,*/
                'ajax': {
                    'url':'phpajax/datagrid_list_all.php?action=proc_qoutation'
                },
                'columns': [
                    { data: 'sl' },
                    { data: 'rfq' },
