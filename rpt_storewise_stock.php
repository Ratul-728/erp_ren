...
function table_with_filter(url){
    ...
    $("#cmbbranch,#cmbcat,#bc,#cmbbrand").on("change input", function() {
            ...
            // Add only the values that are not empty
            if (branch) params.push("branch=" + encodeURIComponent(branch));
            if (brand) params.push("brand=" + encodeURIComponent(brand));
            if (bc) params.push("bc=" + encodeURIComponent(bc));
            if (cat) params.push("cat=" + encodeURIComponent(cat));
            
            // Join the parameters with "&"
            var urlQueryStr = params.join("&");
            $("#urlqeurystring").val(urlQueryStr);
            
            // // If needed, prepend a "?" to make it a proper query string
            // if (urlQueryStr) {
