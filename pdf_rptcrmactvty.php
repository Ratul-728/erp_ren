...
$sl = 1;
$loop = "";
    $qry="SELECT DATE_FORMAT(d.`comndt`,'%e/%c/%Y %h:%i:%s %p') dt,h.hrName ,t.`name` acttp ,c.`name` cus,d.note,d.place alowance,d.status,d.value ,org.name ornm
	...
	<tr>
		<td '.$tdStyle.'>'.$sl.'</td>
		<td '.$tdStyle.' nowrap>'.$rowinv["dt"].'</td>
		<td '.$tdStyle.'>'.$rowinv["hrName"].'</td>
		<td '.$tdStyle.'>'.$rowinv["acttp"].'</td>
		<td '.$tdStyle.'>'.$rowinv["cus"].'</td>
		<td '.$tdStyle.'>'.$rowinv["note"].'</td>
		<td '.$tdStyle.'>'.$rowinv["alowance"].'</td>
		<td '.$tdStyle.'>'.$rowinv["value"].'</td>
		<td '.$tdStyle.'>'.$rowinv["ornm"].'</td>
...
$obj_pdf->SetTitle("Activity Report");
...
<table width="100%" border="0"  cellspacing="0" cellpadding="0">
   <thead>
	<tr>
		...
		<td width="50%" align="right"><h1>Activity Report</h1></td>
	</tr>
	</thead>		
</table>		
<table width="100%" border="1"  cellspacing="0" cellpadding="3">
   <thead>
	<tr>
		<td '.$thStyle.'>SL.</td>
		<td '.$thStyle.'>Date</td>
		<td '.$thStyle.'>Activity Maker</td>
		<td '.$thStyle.'>Activity Type</td>
		<td '.$thStyle.'>Customer</td>
		<td '.$thStyle.'>Note</td>
		<td '.$thStyle.'>Allowence</td>
		<td '.$thStyle.'>transfer Amount</td>
...
$obj_pdf->OutPut("Activity_Report","I");
//echo $content;
}
?>
