...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    if (isset($_POST['add'])) {
        header("Location: " . $hostpath . "/coafinance.php?res=0&msg='Insert Data'&mod=17");
    }
    if (isset($_POST['export'])) {
        ...
        , `ctlgl`, `isposted`,`oflag`, `dr_cr`, `lvl`, `opbal`, `closingbal` FROM `coa` WHERE `status` = 'A'  order by glno";
        // echo  $qry;die;
...
include_once 'menu.php';
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="coafinanceList.php" id="form1">
...
include_once 'common_footer.php';
	...
	url = 'phpajax/datagrid_list_all.php?action=coafinance';
	table_with_filter(url);	
	
        //Status
        $("#cmblvl").on("change", function() {
            var level = $(this).val();
			url = 'phpajax/datagrid_list_all.php?action=coafinance&coalvl='+level;
			//alert(status);
            setTimeout(function(){
			    table_with_filter(url);
            }, 350);			
