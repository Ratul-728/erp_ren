...
if($type == 1 || $type == 3)
{
    ...
    $qryInfo="SELECT org.name, so.orderdate, org.orgcode, org.contactno, so.remarks,MAX(DATE_FORMAT(quow.expted_deliverey_date, '%d/%m/%Y')) expdt 
                ...
                LEFT JOIN `organization` org ON org.id=so.organization WHERE q.order_id = '".$orderId."'";
...
else if($type == 2)
{
    ...
    $qryInfo="SELECT DATE_FORMAT(q.delivery_date, '%d/%b/%Y %H:%i:%s') delivery_date,sup.name supplier
                FROM qa q LEFT JOIN qa_warehouse qaw ON qaw.qa_id=q.id 
                LEFT JOIN suplier sup ON qaw.warehouse_id=sup.id WHERE q.order_id='".$orderId."'";//echo $qryInfo;die;
    $resultInfo = $conn->query($qryInfo);
    while ($rowinfo = $resultInfo->fetch_assoc()) 
    {
        $rcvdt = $rowinfo["delivery_date"];
        $supplier = $rowinfo["supplier"];
        //$expDeliverydt = $rowinfo["expdt"];
    }
}
                                            ...
                                            <div class="well list-top-controls"> 
                                                <div class="row border">
                                                   <div class="col-sm-3 text-nowrap">
                                                        <h6>QA Result <i class="fa fa-angle-right"></i> Purchased Items <i class="fa fa-angle-right"></i> ID: <?= $orderId ?> </h6>
                                                   </div>
                                                ...
                                                <div class="row">
                                                   <div class="col-md-6">
                                                           <b>ORDER DATE:  </b>  <?= $orderDate ?> <br>
                                                            <b>DELIVERY DATE:  <?= $expDeliverydt ?>  </b>   
                                                    </div>
                                                    <div class="col-md-6">
                                    ...
                                    <?php } ?>   
                                        ...
                                        <?php
                                            ...
                                            $qryQa = "SELECT 
                                                        ...
                                                        b.title brandnm
                                                        FROM `qa` q 
                                                        LEFT JOIN item i ON q.product_id=i.id 
                                                        LEFT JOIN `brand` b ON i.brand=b.id 
                                                        LEFT JOIN `itmCat` cat ON cat.id = i.catagory 
                                                        WHERE q.order_id = '".$orderId."' and q.type = ".$type;
                                        ...
                                        ?><tr>
                                                    ...
                                                    <?php 
                                                        $qryQaw = "SELECT 
                                                                    ...
                                                                    qaw.damaged_qty, 
                                                                    qaw.id, 
                                                                    qaw.warehouse_id whid
                                                                    FROM `qa_warehouse` qaw 
                                                    ...
                                                    ?>
                                                                ...
                                                                <?php if($type == 1 && ($whid == 7 || $whid == 8)) { ?>
                                                                    <td class="qswr-nothing">Future/Back Order</td>
                                                                <?php } else { ?>
                                        ...
                                        <?php   } // while ($rowQa = $resultQa->fetch_assoc()) {    ?>
...
<script>
    ...
    function submitQAData(formData){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "phpajax/qa_form_submit.php", // Replace with the URL to your PHP script
                        data: formData,
