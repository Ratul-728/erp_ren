...
$qry = "SELECT s.id, t.name tn, p.name pn, s.freeqty, s.costprice, p.rate mrp, r.name str, s.barcode, s.storerome, p.image
        FROM chalanstock s 
        LEFT JOIN item p ON s.product = p.id 
        LEFT JOIN itmCat t ON p.catagory = t.id 
        LEFT JOIN branch r ON s.storerome = r.id  
        WHERE s.storerome = 7 AND s.freeqty <> 0
        ORDER BY s.id DESC";
...
if ($resultinv->num_rows > 0) {
    while ($row2 = $resultinv->fetch_assoc()) {
        ...
        $pdf->Cell(20, 10, $freeqty, 1); // Quantity
        $pdf->Ln(); // Move to the next line
        $sl++;
    }
} else {
