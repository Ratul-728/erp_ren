...
if ($usr == '') {
    ...
    $currPage    = basename($_SERVER['PHP_SELF']);
       ...
       /* 
        $qryUpperInfo = "SELECT s.`id`,s.orderstatus,s.`socode`,	o.name orgname, p.name projnm,s.srctype,DATE_FORMAT(s.`orderdate`,'%e/%c/%Y') `orderdate`
            FROM `quotation` s 
			left join organization o ON o.id = s.organization 
			left join project p on s.project=p.id
			where  s.orderstatus>=3 and s.socode= '$orderID'";
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-4 col-md-6 col-sm-6">
                                                ...
                                                <div class="input-group">
                                                    <input type="text" class="form-control" name="salsestp" id="salsestp" value="" disabled>
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function() {
      ...
      $('#find').click(function() {
        ...
        $.ajax({
            url:"phpajax/cancelorderajax.php",
            method:"POST",
            data:{orderid: order},
            //dataType: 'JSON',
