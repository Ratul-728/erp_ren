...
from return_order ro LEFT JOIN invoice inv ON inv.soid=ro.order_id
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1><?= strtoupper('Return Order') ?></h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
-->
		...
		<td valign="top">
			<h5>Return Order Informaiton<br></h5>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-items">
  <tbody>
    <tr class="inv-header">
      ...
      <th scope="col" align="center">From Warehouse</th>
      <th scope="col" align="center">To Warehouse</th>
...
<?php
$sosl = 1;
$dqry   = "SELECT i.barcode, i.name product, i.dimension, i.image, ror.return_qty, qaw.ordered_qty, i.description, i.parts, fbr.name fromwarehouse, tbr.name towarehouse
            FROM `return_order_details` ror LEFT JOIN qa_warehouse qaw ON qaw.id=ror.qaw_id LEFT JOIN qa q ON q.id=qaw.qa_id LEFT JOIN item i ON i.id = q.product_id 
            LEFT JOIN `return_order` ro ON ro.id=ror.ro_id LEFT JOIN branch fbr ON fbr.id=qaw.warehouse_id LEFT JOIN branch tbr ON tbr.id=ror.warehouse
            WHERE ro.ro_id = '".$roid."'";
...
if ($resultd->num_rows > 0) {
    while ($rowsd = $resultd->fetch_assoc()) {
        ...
        $pnm                             = $rowsd["product"];
        $orderqty                        = $rowsd["ordered_qty"];
        $returnqty                       = $rowsd["return_qty"];
        $parts                           = $rowsd["parts"];
    ...
    <tr>
      ...
      <td>
		...
		<td align="center"><?=$fromwarehouse?></td>
		<td align="center"><?=$towarehouse?></td>
      <td align="center"><?=$orderqty?></td>
      <td class="number"><?= $returnqty?></td>
<?php $sosl++;}} ?>
    </tr>
  </tbody>
</table>
