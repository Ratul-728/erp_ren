...
else
{
    ...
    */
    $currSection = 'dealforcast';
    $currPage = basename($_SERVER['PHP_SELF']);
    
    if ( isset( $_POST['add'] ) ) {
           header("Location: ".$hostpath."/rpt_deal_forcast.php?res=0&msg='Insert Data'");
    }
    if ( isset( $_POST['export'] ) )
    {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('M1', 'MRC')
                 ...
                 ->setCellValue('R1', 'Revenue')
                 ->setCellValue('S1', 'Scale')
                 ->setCellValue('T1', 'Probability'); 
    			
        $firststyle='A2';
        $qry="SELECT s.`name` socode, s.`contType`, s.`cus_id`, s.`cus_nm`, s.`orderdate`, s.`yr`, s.`mnth`, s.`dy` da, s.`hrid`, s.`hrName`, s.`itmid`, s.`itmnm`, s.`otc`, s.`mrc`
    , s.`stage`, s.`prob`, s.`itm_cat`, s.`size`, s.`pattern`, s.`orgn`,r.yr,r.month,r.dy,r.`dt`
,s.`st` stat
,round(s.`mrc`,2) pmrc
,round(s.`otc`,2) otcvalue
,'Forcast'  frcst
,s.`scale`,round(s.`probability`,0)probability,round(((s.`otc`+s.`mrc`)*s.probability/100),2) revenue,s.currency
FROM  `rpt_sales_deal` s  ,`reportmanth` r  
WHERE   ((r.yr=s.yr and r.month>=s.mnth) or (r.yr>s.yr))
order by s.`name`,s.`itmnm`,r.`dt` asc"; 
       // echo  $qry;die;
       //s.`socode`='ANTGR003' and  
        ...
        { $i=0;
            ...
            { 
                $urut=$i+2;	$col1='A'.$urut; $col2='B'.$urut;$col3='C'.$urut;$col4='D'.$urut;$col5='E'.$urut;$col6='F'.$urut;$col7='G'.$urut;$col8='H'.$urut;$col9='I'.$urut;$col10='J'.$urut;$col11='K'.$urut;$col12='L'.$urut;$col13='M'.$urut;$col14='N'.$urut;$col15='O'.$urut;$col16='P'.$urut;$col17='Q'.$urut;$col18='R'.$urut;$col19='S'.$urut;$col20='S'.$urut;
                $i++; 
                    $objPHPExcel->setActiveSheetIndex(0)
    					        ...
    					        ->setCellValue($col12, $row['currency'])
        					    ...
        					    ->setCellValue($col17, $row['frcst'])
        					     ->setCellValue($col18, $row['revenue'])
        					    ->setCellValue($col19, $row['scale'])
        					    ->setCellValue($col20, $row['probability']);	/* */
        			$laststyle=$title;	
                }
            }
      
        $objPHPExcel->getActiveSheet()->setTitle('deal_forcast');
        $objPHPExcel->setActiveSheetIndex(0);
        $today=date("YmdHis");
        $fileNm="data/".'deal_forcast_'.$today.'.xls'; 
          ...
          <div class="row">
            ...
            <p>&nbsp;</p>
              ...
              <div class="panel panel-info">
      			<div class="panel-heading"><h1>All Deals</h1></div>
    				<div class="panel-body">
    
    <span class="alertmsg">
    </span>
    <br>
                	<form method="post" action="rpt_deal_forcast.php" id="form1">
                        ...
                        <table  class="table table-grid table-striped table-hover">
                                <thead>
                                     <tr class="filters">
                                        ...
                                        <th><a href="#">RevenueC</a></th>
                                        <th><a href="#">Scale</a></th>
                                        <th><a href="#">Probality %</a></th>
                                        <th>&nbsp;</th>
                                    </tr>
                                </thead>
                                <tbody>       
...
FROM  `rpt_sales_deal` s  ,`reportmanth` r  
WHERE   ((r.yr=s.yr and r.month>=s.mnth) or (r.yr>s.yr))
order by s.`name`, s.`itmnm`,r.`dt` asc LIMIT ".$limitst. ",".$limitnd;
    $sl=0;
