...
if ($usr == '') {
	...
	$qry = "SELECT DATE_FORMAT( tansferdt,'%d/%b/%Y') tansferdt from transfer_stock where toid = '$toid'";
	$result = $conn->query($qry);
    while ($row = $result->fetch_assoc()) {
        $transferdt = $row["tansferdt"];
    }
}
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-header">
  <tbody>
    <tr>
	  ...
	  <td align="right"><h1>Transfer Order</h1></td>
    </tr>
  </tbody>
</table>
<hr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-items">
  <tbody>
    <tr class="inv-header">
      ...
      <th scope="col" class="number">From Store</th>
      <th scope="col" class="number">To Store</th>
      <th scope="col" class="number">Quantiy</th>
      <th scope="col" class="number">Approval Quantiy</th>
    </tr>
	  
	  
<?php

$quotDetailQry ="SELECT tsd.qty, frmbr.name frombranch, tobr.name tobranch, i.name product,i.description description,i.image,i.barcode,i.dimension, tsd.`approval_qty`, i.parts
                FROM `transfer_stock` ts LEFT JOIN transfer_stock_details tsd ON ts.id=tsd.toid LEFT JOIN branch frmbr ON tsd.from_store=frmbr.id 
                LEFT JOIN branch tobr ON tobr.id=tsd.to_store LEFT JOIN item i ON i.id=tsd.product
                WHERE ts.toid = '" . $toid. "'";	
...
if ($resultd->num_rows > 0) {
    while ($rowsd = $resultd->fetch_assoc()) {
        ...
        $fromstore                             = $rowsd["frombranch"];
        $tostore                          = $rowsd["tobranch"] ;
        $qty                          = $rowsd["qty"] ;
        $code                            = $rowsd["barcode"];
		$parts                       = $rowsd["parts"];
		$desc                            = $rowsd["description"];
		$appqty                          = $rowsd["approval_qty"];
    ...
    <tr>
      ...
      <td class="number"><?=$fromstore?></td>
      <td class="number"><?=$tostore?></td>
      <td class="number"><?=$qty?></td>
      <td class="number"><?=$appqty?></td>
    </tr>
<?php }} ?>
