...
if ($usr == '') {header("Location: " . $hostpath . "/hr.php");
    ...
    while ($row = $resultchorg->fetch_assoc()) {
            ...
            $adjustper = ($row['financadjstmnt'])?$row['financadjstmnt']:'NA';
            $adjamount = ($row['adjustedamount'])?$row['adjustedamount']:'NA';
...
.txt-value-e span:nth-child(2),
.txt-value-e span:nth-child(4){
    font-weight: normal!important;
    font-size: 10px;
    line-height: 2.5;
    padding: 0px 5px;
}
.txt-value-e span:nth-child(3) input,
.txt-value-e span:nth-child(5) input{
    width: 50px;
    margin: 0px 5px;
    padding: 3px;
	height: 25px;
	...
	<table class="tbl-top1" width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
		  <td class="logo"><img src="<?=$hostpath?>/assets/images/site_setting_logo/<?=fetchByID('sitesettings',id,1,'doc_header_logo');?>"></td>
			<td><h1>Financial Customization of Payment Receipt #<?= $trid ?></h1></td>
		</tr>
	</table>
	...
	<table class="tbl-body1" width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
		  ...
		  <td width="33%" class="pl-1">
			  ...
			  <div class="txt-value-e"><span id="amount"><?=$amount?> Tk. </span><span>|</span> <span><input type="text" name="reserved" id="reserved"  value="<?=$adjustper?>">% </span><span>|</span> <span><input type="text"  name="taka_reserved" id="taka_reserved" readonly value=<?=$adjamount?>></span></div>
				  <hr class="light">
...
$(document).ready(function(){
    $('#reserved').on("input", function(){
        var amount = parseFloat($('#amount').text()); // Parse amount as a float
        var reserved = parseFloat($(this).val()); // Parse reserved as a float

        // Check if both amount and reserved are valid numbers
        if (!isNaN(amount) && !isNaN(reserved)) {
            var amt = (amount * reserved) / 100;
            $('#taka_reserved').val(amt);
        } else {
            $('#taka_reserved').val(''); // Clear the field if input is invalid
        }
    });
});
</script>
