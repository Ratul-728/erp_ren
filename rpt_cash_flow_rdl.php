  ...
  if ( isset( $_POST['export'] ) ) 
  {
       'As on '.date("F", strtotime($pmn)) .','.$pyr;
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('B2', 'Profit Loss Statement')
                ->setCellValue('B3', 'For the Moth '.date("F", strtotime($pmn)).' , '.$pyr.'')
                ->setCellValue('A4', '')
                ->setCellValue('B4', 'Amount')
                ->setCellValue('A5', 'INCOME')
                ->setCellValue('B4', ''); 
    			
        $firststyle='A5';
        //assets
        $elvl2="SELECT c.glno,c.glnm,sum(c.closingbal) closingbal FROM `coa_mon` c  where c.ctlgl='300000000' and c.yr='$pyr' group by c.glno,c.glnm ";
        ...
        if ($eresult2->num_rows > 0) 
        {
            while ($erow2 = $eresult2->fetch_assoc())
            {
			    ...
			    $elvl3="SELECT c.glno,c.glnm,sum(c.closingbal) closingbal FROM `coa_mon` c  where c.ctlgl=$egl2 and c.yr='$pyr' group by  c.glno,c.glnm";
    ...
    $elvl1="SELECT c.glno,c.glnm,sum(c.closingbal) closingbal FROM `coa_mon` c  where c.glno='300000000' and c.yr='$pyr' group by  c.glno,c.glnm ";
    ...
    if ($eresult1->num_rows > 0)
    {
        ...
        $urut=$i+5;	$col1='A'.$urut; $col2='B'.$urut;
        $objPHPExcel->setActiveSheetIndex(0)
		            ->setCellValue($col1,'Total Income')
	...
	//liability
        ...
        $urut=$i+5;	$col1='A'.$urut; $col2='B'.$urut;
        $objPHPExcel->setActiveSheetIndex(0)
		            ->setCellValue($col1,'Expense')
		            ->setCellValue($col2, '');
		            
	 $llvl2="SELECT c.glno,c.glnm,sum(c.closingbal) closingbal FROM `coa_mon` c  where c.ctlgl='200000000' and c.yr='$pyr' group by  c.glno,c.glnm ";
        ...
        if ($lresult2->num_rows > 0) 
        {
            while ($lrow2 = $lresult2->fetch_assoc())
            {
			    ...
			    $llvl3="SELECT c.glno,c.glnm,sum(c.closingbal) closingbal FROM `coa_mon` c  where c.ctlgl=$lgl2 and c.yr='$pyr' group by  c.glno,c.glnm ";
    ...
    $llvl1="SELECT c.glno,c.glnm,sum(c.closingbal) closingbal FROM `coa_mon` c  where c.glno='400000000' and c.yr='$pyr' group by  c.glno,c.glnm";
            ...
            <div class="row">
                <div class="col-lg-12" >
                    ...
                    <p>
                     ...
                     <form method="post" action="rpt_cash_flow_rdl.php?pg=1&mod=7" id="form1" enctype="multipart/form-data">  
...
<?php } ?>                    
                            ...
                            <?php include_once('phpajax/rpt_load_cash_flow_rdl.php'); ?> 
        <!-- /#end of panel -->
                    </form>
