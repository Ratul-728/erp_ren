...
else
{
    ...
    if($postaction == 'Save' || $postaction == 'Revision' || $postaction == 'Order'){
        ...
        if($mode == 'insert'){
            ...
            //check if $_POST['refid'] found, update it with Quotation ID;
             
            if($_POST['refid']){
                //$debug = 1;
                $condition = "order_id ='".$_POST['refid']."'";
                //updateByID($tableName,$colToUpdate,$updatableValue,$condition)
                
                $result = updateByID('approval_quotation_price_change','order_id',"'$poid'",$condition);
                if($result){
                    file_put_contents('rakplanet.txt',$condition);
                }
            }
 ...
 //die;
            ...
            if($postaction == 'Save' || $postaction=='Order'){
                if($postaction == 'Order'){
                    //Static mail
                    $qrySalesHead = "SELECT concat(emp.firstname, ' ', emp.lastname) empnm, emp.office_email 
                                    FROM `department` d LEFT JOIN employee emp ON d.head = emp.id 
                                    WHERE d.id = 2";
                    $resultSH = $conn->query($qrySalesHead);
                    while($rowsh = $resultSH->fetch_assoc()){
                        if($rowsh["office_email"] != ""){
                            $recipientNames = array();
                            $recipientEmails = array();
                            $ccEmails = array();
                            $qryAssistantManager = "SELECT concat(emp.firstname, ' ', emp.lastname) empname, emp.office_email 
                                                    FROM `employee` emp LEFT JOIN hr h ON h.emp_id = emp.employeecode 
                                                    WHERE h.active_st = 1 AND designation = 55 AND department = 5";
                            $resultAM = $conn->query($qryAssistantManager);
                            while($rowam = $resultAM->fetch_assoc()){
                                $ccEmails[] = $rowam["office_email"];
                            }
                            
                            $recipientNames[] = $rowsh["empnm"];
                            $recipientEmails[] = $rowsh["office_email"];
                            
                            $mailsubject = "New order created";
                            $message = "Dear ".$rowsh["empnm"].",<br>
                                        A new order($poid) is  created. Please proceed with payment collection. 
