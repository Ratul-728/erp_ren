...
else
{
   ...
   // if ( isset( $_POST['add'] ) || isset( $_POST['addprint'] )){
	...
	{ 
        ...
        if (is_array($item))
        {
		    for ($i=0;$i<count($item);$i++)
            {
                ...
                $itqry="insert into soitemdetails( `socode`,`sosl`, `productid`,`remarks`, `mu`, `qty`, `deliveredqty`, `dueqty`,vatrate,`vat`, `otc`,aitrate, `ait`,cost,discountrate,discounttot,`currency`, `makeby`, `makedt`)
                        values( '".$poid."','".$itmsl."','".$itmmnm."','".$descr."','".$mu."','".$qty."','0','".$qty."','".$vatrt."','".$vata."','".$upo."','0','0',".$prdcost.",".$disc.",".$disctot.",'".$currnm."','".$hrid."','".date("Y-m-d H:i:s")."')";
                ...
                if($qty!='')
                {
                    ...
                    values('".$poid."','".$invno."','".$itmsl."',1,'".$invmn."','".$invyr."',STR_TO_DATE('".$invdt."', '%d/%m/%Y'),'".$itmmnm."','".$qty."','".$upo."',".$disc.",".$disctot.",'".$vata."','0','".$currnm."','".$hrid."','".date("Y-m-d H:i:s")."')";
                    // echo $qryinvdet;die;
        ...
        values($orderstatus,'".$poid."','".$custp."','".$org."','".$srctp."','".$sup_id."',STR_TO_DATE('".$po_dt."', '%d/%m/%Y'), '".$deliveryby."',".$deliveryamt.",".$discntnt.",'".$hrid."','".$vat."','".$tax."','".$newInvoiceAmount."','".$hrid."','".date("Y-m-d H:i:s")."','".date("Y-m-d H:i:s")."','".$st."','".$det."','".$poc."')";
        ...
        values('".$invno."',STR_TO_DATE('".$invdt."', '%d/%m/%Y'),'".$invyr."','".$invmn."','".$poid."','".$org."','".$newInvoiceAmount."','".$newInvoiceAmount."',".$discntnt.",0,'".$newInvoiceAmount."',1,1,'".$hrid."','".date("Y-m-d H:i:s")."')";
        // echo $qryinv;die;
        ...
        $orgwallet="insert into organizationwallet(`transdt`,`orgid`,`transmode`,`dr_cr`,`trans_ref`,`amount`,`remarks`,`makeby`,`makedt`)
            values(STR_TO_DATE('".$invdt."', '%d/%m/%Y'),'$org','Auto','na','$invno','$invamt','Invoice Generated','$hrid','".date("Y-m-d H:i:s")."')";
        //echo $itqry;die;
        if ($conn->query($orgwallet) == TRUE) { $err="organization Wallet updated successfully";  }
         
        $note="Service Order: SO ID:".$poid." with amount ".$newInvoiceAmount." is ".$ordermsg;
        $qry_othr="insert into comncdetails(`contactid`,`comntp`, `comndt`, `note`, `place`, `status`, `value`, `makeby`, `makedt`) 
        values('".$org."',5,STR_TO_DATE('".$po_dt."', '%d/%m/%Y %h:%i %p'),'".$note."','',0,'".$newInvoiceAmount."','".$hrid."','".date("Y-m-d H:i:s")."')" ;
        //echo $qry_othr;die;
         if ($conn->query($qry_othr) == TRUE) { $err="Order updated successfully";  }
         
         $qry_othr="insert into comncdetails(`contactid`,`comntp`, `comndt`, `note`, `place`, `status`, `value`, `makeby`, `makedt`) 
        values('".$sup_id."',5,STR_TO_DATE('".$po_dt."', '%d/%m/%Y %h:%i %p'),'".$note."','',0,'".$newInvoiceAmount."','".$hrid."','".date("Y-m-d H:i:s")."')" ;
        ...
        $glmqry="INSERT INTO `glmst`(`vouchno`, `transdt`, `refno`, `remarks`, `entryby`, `entrydate`) 
                    VALUES ('".$vouch."',STR_TO_DATE('".$po_dt."', '%d/%m/%Y'),'".$poid."-".$sup_id."','".$det."','".$hrid."','".date("Y-m-d H:i:s")."')";
        ...
        //Updtae Voucher
            ...
            $glqry1="INSERT INTO `gldlt`(`vouchno`, `sl`, `glac`, `dr_cr`, `amount`,`remarks`, `entryby`, `entrydate` ) 
                            VALUES ('".$vouch."',1,'".$glnoinv."','D','".$newInvoiceAmount."','Sales Order','".$hrid."','".date("Y-m-d H:i:s")."')
                            ,('".$vouch."',2,'".$glno."','C','".$tot_amt."','SO','".$hrid."','".date("Y-m-d H:i:s")."')
                            ,('".$vouch."',3,'203020102','C','".$totvat."','SO','".$hrid."','".date("Y-m-d H:i:s")."')
                            ,('".$vouch."',4,'301020204','C','".$deliveryamt."','SO','".$hrid."','".date("Y-m-d H:i:s")."')";
                                        //echo  $glqry1;die;// ,('".$vouch."',4,'203020103','C','".$totait."','SO','".$hrid."','".date("Y-m-d H:i:s")."')              
	...
	{ 
        ...
        $qry="update soitem set `orderstatus`=$orderstatus, `srctype`='".$srctp."',`customertp`='".$custp."',`organization`='".$org."',`customer`='".$sup_id."',`orderdate`=STR_TO_DATE('".$po_dt."', '%d/%m/%Y'),`accmanager`='".$acc_mgr."' ,`deliveryamt`='".$deliveryamt."' 
            ,`remarks`='".$det."',`status`='".$st."' ,`oldsocode`='".$oldso."',`adjustment`='".$discntnt."',`makedt`='".date("Y-m-d H:i:s")."',`lastdeliverydt`='".date("Y-m-d H:i:s")."'
            ,`vat`='".$vat."' ,`tax`='".$tax."',`invoiceamount`='".$newInvoiceAmount ."' where `socode`='".$poid."'";
            $err="SO updated successfully";
            
        $orgwallet="insert into organizationwallet(`transdt`,`orgid`,`transmode`,`dr_cr`,`trans_ref`,`amount`,`remarks`,`makeby`,`makedt`)
                    values(STR_TO_DATE('".$invdt."', '%d/%m/%Y'),'$org','Auto','na','$invno','$invamt','Invoice re Generated','$hrid','".date("Y-m-d H:i:s")."')";
        //echo $itqry;die;
        if ($conn->query($orgwallet) == TRUE) { $err="organization Wallet updated successfully";  }    
            
       
        $note="Service Order: SO ID".$poid." with amount ".$newInvoiceAmount." was updated";
        $qry_othr="insert into comncdetails(`contactid`,`comntp`, `comndt`, `note`, `place`, `status`, `value`, `makeby`, `makedt`) 
        values('".$sup_id."',5,STR_TO_DATE('".$po_dt."', '%d/%m/%Y %h:%i %p'),'".$note."','',0,'".$newInvoiceAmount."','".$hrid."','".date("Y-m-d H:i:s")."')" ;
        //echo $qry_othr;die;
         if ($conn->query($qry_othr) == TRUE) { $err="Order updated successfully";  } 
        ...
        //Updtae Voucher
            ...
            $glqry1="INSERT INTO `gldlt`(`vouchno`, `sl`, `glac`, `dr_cr`, `amount`,`remarks`, `entryby`, `entrydate` ) 
                                        ...
                                        ,('".$vouch."',3,'203020102','C','".$totvat."','SO','".$hrid."','".date("Y-m-d H:i:s")."')
                                        ,('".$vouch."',4,'301020204','C','".$deliveryamt."','SO','".$hrid."','".date("Y-m-d H:i:s")."')
                                        ,('".$vouch."',5,'".$glnoinv."','C','".$oldinvamt."','Sales Order','".$hrid."','".date("Y-m-d H:i:s")."')
                                        ,('".$vouch."',6,'".$glno."','D','".($oldinvamt-$oldvatamt-$olddelvamt)."','SO','".$hrid."','".date("Y-m-d H:i:s")."')
                                        ,('".$vouch."',7,'203020102','D','".$oldvatamt."','SO','".$hrid."','".date("Y-m-d H:i:s")."')
                                        ,('".$vouch."',8,'301020204','D','".$olddelvamt."','SO','".$hrid."','".date("Y-m-d H:i:s")."')";
                                        //echo  $glqry1;die;              
            if ($conn->query($glqry1) == TRUE) { $err="GL Master added successfully";  }else{ $errflag++;}
        }
       /* Accounnting */
    ...
    //if ( isset( $_POST['update'] ) ) {
        ...
        if ($conn->query($qry) == TRUE) 
        {
			...
			if($_REQUEST['postaction']=='Confirm'){
				...
				//1 job: return to list;
				$err = "Order booked successfully";
				header("Location: ".$hostpath."/invoiceList.php?changedid=".$invno."&mod=3");
			}				
