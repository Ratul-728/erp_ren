        ...
        if($mode == 'insert'){
            ...
            foreach($inputOrderDetailData as $quotationDetailData){
                ...
                for($m = 0; $m < count($whqty); $m++){
                    ...
                    $found = fetchTotalRecordByCondition('quotation_warehouse','socode = "'.$poid.'" AND pid = '.$quotationDetailData['productid'].' AND warehouse='.$whid[$m],'*');
                    if($found < 1){
            ...
            foreach($inputOrderDetailData as $quotationDetailData){
                ...
                for($m = 0; $m < count($whqty); $m++){
                   ...
                   // $inputWarehouseData[$m] = array('TableName' => 'quotation_revisions_warehouse','soitem_detail_id' => $insertId);
                    ...
                    $found = fetchTotalRecordByCondition('quotation_revisions_warehouse','socode = "'.$poid.'" AND pid = '.$quotationDetailData['productid'].' AND warehouse='.$whid[$m],'*');
                    if($found < 1){
                        insertData($inputWarehouseData,$msg,$success,$insertIdWH);
                    }
                    $response['result'][] = $success;
            ...
            if($mode == 'insert'  || $mode == 'update'){
                ...
                foreach($inputOrderDetailData as $quotationDetailData){
                    ...
                    for($m = 0; $m < count($whqty); $m++){
                        ...
                        $found = fetchTotalRecordByCondition('soitem_warehouse','socode = "'.$poid.'" AND pid = '.$quotationDetailData['productid'].' AND warehouse='.$whid[$m],'*');
                        if($found < 1){
                            insertData($inputWarehouseData,$msg,$success,$insertIdWH);
                        }
                        $response['result'][] = $success;
                ...
                if($postaction == 'Order'){
                    ...
                    while($rowmail = $resultmail->fetch_assoc())
                    {
                        ...
                        if($active == 1)
                        {
                            ...
                            $message = "A new order($poid) is  created. Please proceed with payment collection.";
                            
                            sendBitFlowMailArray($recipientNames, $recipientEmails, $mailsubject, $message, $ccEmails);
                        }
                    }
                    //Mail to Customer
                    $qrymail = "SELECT active FROM `email` WHERE id = 55";
                    $resultmail = $conn->query($qrymail);
                    while($rowmail = $resultmail->fetch_assoc())
                    {
                        $active = $rowmail["active"];
                        if($active == 1)
                        {
                            ...
                            $message = "We recieved an order for order id $poid <br><br>
