...
else
{
    ...
    */
    ...
    if ( isset( $_POST['add'] ) ) {
           header("Location: ".$hostpath."/leavetype.php?res=0&msg='Insert Data'&mod=4");
    }
   if ( isset( $_POST['export'] ) ) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('C1', 'DAYS FOR PERMANENT')
                ->setCellValue('D1', 'DAYS FOR CONTRACTUAL')
                ->setCellValue('E1', 'DESCRIPTION');
                  
    			
        $firststyle='A2';
        $qry="SELECT `id` id, `title`,`day`, `remarks` FROM `leaveType`"; 
       // echo  $qry;die;
        ...
        { $i=0;
            ...
            { 
                ...
                $objPHPExcel->setActiveSheetIndex(0)
    						...
    						->setCellValue($col3, $row['day'])
    						->setCellValue($col4, $row['day_contractual'])
    					    ->setCellValue($col5, $row['remarks']);
    					    	/* */
    			//$laststyle=$title;	
            }
        }
        $objPHPExcel->getActiveSheet()->setTitle('LEAVE TYPE');
        $objPHPExcel->setActiveSheetIndex(0);
        $today=date("YmdHis");
        $fileNm="data/".'leave_type'.$today.'.xls'; 
    ...
    <span class="alertmsg">
                	...
                	<form method="post" action="leavetypeList.php" id="form1">
            
                     <div class="well list-top-controls"> 
                      <div class="row border">
                       
                         <div class="col-sm-3 text-nowrap">
                            <h6>HRM <i class="fa fa-angle-right"></i>All Leave Type</h6>
                       </div>
                ...
                <div >
                    ...
                    <table id='listTable' class='display dataTable actionbtn firstcolpad0' width="100%">
                        ...
                        <tr>
                            <th>SL.</th>
                            <th>Title</th>
                            <th>Days for Permanent</th>
                            <th>Days for Contractual</th>
                            <th>DESCRIPTION </th>
                            <th></th>
     ...
     <!-- Datatable JS -->       
        ...
        $(document).ready(function(){
            var table1 = $('#listTable').DataTable({
				...
				/*'searching': true,*/
                'ajax': {
                    'url':'phpajax/datagrid_list_all.php?action=leavetype'
                },
                'columns': [
                    { data: 'sl' },
                    { data: 'title' },
                    { data: 'day' },
                    { data: 'day_contractual' },
